...
| **Scaffold Variant (this generation)** | `pickScaffoldVariant()` + promptblocket `"## Scaffold Variant (this generation)"` | `scaffold-variants/matcher.ts`, `scaffold-variants/registry.ts`, `system-prompt.ts` | Scaffold-bunden visuell variant per request: signaturmotiv, fontpar, prompt-hints, dossier-härledda style-rules/sectionInventory/avoidPatterns/`worldClassRubric`, och variant-defaults för tema-tokens. | kanonisk |
| **Variant Structural Files** | `selectVariantStructuralFiles()` + `selectCapabilityStructuralFiles()` + promptblocket `"## Structural References (this variant)"` | `scaffold-variants/structural-files.ts`, `template-library/catalog.ts`, `system-prompt.ts` | Init-only strukturella kodreferenser: (1) variant-driven filer från variantens `sourceTemplateIds`, (2) capability-driven filer från hela template-library-katalogen baserat på `InferredCapabilities` (auth, ecommerce, dashboard etc.). Styrs av `SAJTMASKIN_VARIANT_STRUCTURAL_FILES`. | kanonisk |
...
| **Component References** | promptblocket `"## Component References"` | `system-prompt.ts`, `data/shadcn-example-map.ts`, `data/shadcn-example-loader.ts`, `data/shadcn-registry-fetch.ts`, `data/community-registry-fetch.ts` | Tre lager: (1) 33 lokala försynkade exempel (`data/shadcn-examples/`), (2) on-demand HTTP-fetch mot officiella shadcn-registret (radix-vega med new-york-v4 fallback), (3) community-registries (`@shadcnblocks`, `@tailark`, `@magicui` via `config/community-registries.json`). Capability-driven + prompt-keyword-driven matchning. Max 8 officiella + 3 community per generation. Prunas vid tight budget (priority 76). | kanonisk |
...
| 2026-04-14 | Variant Structural Files (v16): +1 term (Variant Structural Files). Scaffold-varianter kan nu injicera budgeterade `selectedFiles`-utdrag (`layout.tsx`, `page.tsx`, `middleware.ts`) från template-library via promptblocket `## Structural References (this variant)`. Ny env-flagga `SAJTMASKIN_VARIANT_STRUCTURAL_FILES`; backoffice visar och styr funktionen. |
| 2026-04-14 | Capability-driven structural refs (v17): `selectCapabilityStructuralFiles()` söker i hela template-library-katalogen baserat på `InferredCapabilities` (needsAuth → signals.auth, needsEcommerce → signals.ecommerce, needsAppShell → signals.dashboard) och injicerar upp till 2 extra filer utöver variantens 3. Merged i `orchestrate.ts` via `mergeStructuralFiles()`. |
| 2026-04-14 | Config + schema + backoffice (v18): Fil-prioriteter och capability-signal-mappning flyttade till `config/structural-file-priorities.json`. Sidebar och header nu tillåtna (prio 15/10). Backoffice Scaffold Lifecycle visar prioriteter, budgetar och capability-mappning. Strikt schema `structural-references.schema.json` i `docs/schemas/strict/`. needsForms→cms och needsCarousel→ecommerce tillagda. |
