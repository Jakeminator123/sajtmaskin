...
import {
  HemsidaUtanKodContent,
  readyRelatedSeoLandingSlugs,
} from "./hemsida-utan-kod-content";
...
describe("HemsidaUtanKodContent", () => {
  it("renders one primary heading, product CTA and only ready related links", () => {
    const entry = getSeoLandingEntry("hemsida-utan-kod");
    render(<HemsidaUtanKodContent />);

    const headings = screen.getAllByRole("heading", { level: 1 });
    expect(headings).toHaveLength(1);
    expect(headings[0]?.textContent).toBe(entry.plannedH1);

    const ctaLinks = screen.getAllByRole("link", { name: "Börja utan kod" });
    ...
    expect(readyRelated).toEqual(
      expect.arrayContaining(["skapa-hemsida", "skapa-hemsida-med-ai", "hemsideprogram"]),
    );
