...
- [~] **Område 6 (§6 steg 4) pågår:** `6-1` event-bus-status-flip för shell + preview-empty-state **mergad** (#159, `b9eb43297`) — ny mapper `version-status-display.ts` (false-green-vakt: degraderad ≠ grön), `useVersionStatus` inkopplad. Kvar: `6-2` (flippa `VersionHistory`, server-enricha `/versions`) + `6-3` (aktivera S3-invarianten). Stänger N#6/G#32 stegvis.
