...
5. **Route by task.** Use `<luna>` only for the mechanical scan lanes and
   `<sol>` for distillation/prioritering and falsification. Resolve both via
   [`subagent-models.mdc`](../../../.cursor/rules/subagent-models.mdc); never let
   an invalid slug inherit the parent model.
...
| 2, 4, 6 … | **Falsification** | one per unverified finding, max 8 | `<sol>`  | try to prove the finding is **not** a bug |
...
2. **Launch.** Parallel `Task` calls (`subagent_type: explore`, `readonly: true`, `model: <sol>`) with the falsification prompt below.
