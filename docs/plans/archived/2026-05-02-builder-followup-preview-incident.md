...
| 2026-05-02 | **A klart:** `/finalize-design` stoppar explicit stale designversion med `409 stale_design_version` om en nyare preferred designversion finns, så F3 inte tyst forkas från äldre bas. F3-triggern visar nu en specifik `Nyare designversion finns`-warning i UI. Verifierat med route-/trigger-tester + `build-spec`/`tier3-build-spec` och `npm run typecheck`. |
