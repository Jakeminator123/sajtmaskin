...
- **Alla PR:er går mot `master`** (trunk) — ingen direktcommit/-push till master. Kör `/granska-diff` före PR/push. Se [`git.mdc`](.cursor/rules/git.mdc) → "Branch-modell".
...
**Before opening the PR:** run `/granska-diff` (8 `composer-2.5-fast` subagents on your own diff) as a pre-filter — also before any push to master. See [`git.mdc`](.cursor/rules/git.mdc). It does **not** replace the post-check below.
