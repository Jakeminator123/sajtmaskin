...
| 2 | ~~AI-sidan anropar aldrig sitt eget `/api/chat`~~ **FIX i PR (branch `fix/f2-mute-brief-fallback`):** F2 mute + brief-fallback återinjicerade `openai-chat`; ownership skippade befintlig lokal demo-panel. Fix: `disableBriefFallback` när mute filtrerat + ownership rewire-check + dossier-panel AI SDK v5 | routen streamar riktigt OpenAI-svar vid direktanrop; sidans HTML har 0 träffar på `api/chat` och texten "Detta är ett lokalt demosvar" | `resolve-base.ts`, `dossiers.ts`, `data/dossiers/hard/openai-chat/` |
...
| 4 | ~~Dossiern `openai-chat` är internt inkonsekvent~~ **FIX i samma PR som #2** (panel → `DefaultChatTransport` / v5) | `chat-panel.tsx` = AI SDK v4-API, `route.ts` = v5 | `data/dossiers/hard/openai-chat/` |
