...
| Simple website path | konservativ snabb init-lane för korta website/template-prompts utan brief, attachments, custom system, multi-route, integrations/contracts eller heavy capabilities | `src/lib/api/engine/chats/simple-website-path.ts`, `create-chat-stream-post.ts`, `orchestrate.ts` | rå prompt + keyword pre-match scaffold + capability flags | `simpleWebsitePath=true/false` + devLog `orchestration.simple_website_path`; när true: ingen Server Auto-Brief, inga externa/UI Recipes, ingen dossier selection | för bred predicate skulle tappa brief/dossiers; därför allowlistade scaffolds och flera blockers |
...
| Capability inference | snabb flaggning av produkt-/UI-capabilities (motion, dekorativ 3D, physics-3d, parallax, payments, auth, forms, ecommerce, charts, app shell, data UI, carousel, command search, theme toggle m.m.) | `src/lib/gen/capability-inference.ts`, `src/lib/gen/capability-dossier-bridge.ts` (init-spår); `src/lib/builder/follow-up-capability-detection.ts` + `follow-up-capability-vocabulary.ts` (follow-up dossier-mapning, separat tröskel) | prompt | `InferredCapabilities` + bridgeade dossier-capabilities (`visual-3d`, `physics-3d`, `parallax-*`, `payments`, `auth`, ...) | falska positiva på ecommerce/app shell/database; förväxling mellan dekorativ floating 3D och faktisk physics; tre parallella signalägare (init-inference, follow-up-detection, scaffold-unlock i `follow-up-clarification.ts`) — regressionsmatris i `follow-up-clarification.test.ts` skyddar gränserna |
