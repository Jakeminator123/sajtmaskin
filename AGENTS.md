...
- The author then posts the sign-off line **and only then** applies the **`merge:ready`** label (that order matters — `merge-ready-freshness.yml` strips a label applied before its sign-off exists; see [`pr-merge-review-gate.mdc`](.cursor/rules/pr-merge-review-gate.mdc)); the **merge-agent** (a Cursor agent) verifies the label + gate and runs `gh pr merge`. There is **no** dashboard auto-merger in the flow (decision 2026-07-09: agent-merger) — see [`auto-merge-automation.mdc`](.cursor/rules/auto-merge-automation.mdc) → "Vem mergar".
