...
The `changeScope` field in the build spec indicates the expected scope of changes:
- `copy` / `local-layout` — small, targeted edits. Return 1-3 files.
- `page-addition` — add new routes/pages. Return new files + minimally updated nav/layout.
- `integration` — add a new integration (Stripe, auth, etc.). Return integration files + updated routes.
- `redesign` — broader visual/structural changes. More files allowed, but still prefer targeted over full regen.
