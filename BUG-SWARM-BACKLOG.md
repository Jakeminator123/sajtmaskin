...
> **Uppdatering 2026-06-23 (`fix/fast-edit-lane-hardening`):** FEL-1, FEL-3, FEL-4, FEL-5 och FEL-6 är **fixade** i den branchen (klienten trådar `engineLatestKnownVersionId`; hosten re-checkar expected-base i låset + rullar tillbaka sessionen vid patch-fail; `booting` tar restart-vägen; env-flaggorna i `serverSchema`). Dessutom #4-härdning: `.env*`/secrets/lockfiles blockas nu i quick-edit-lanen. **FEL-2 kvarstår** (composer `baseVersionRef`-chaining; stale-base-signalen skickas nu men ref-chainingen är inte gjord).
