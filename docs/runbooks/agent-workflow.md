...
kontobaserade fallbacken kräver först en `github-actions[bot]`-ägd
fallbackbegäran för exakt live head och en tillåten orsak (`openai_key_missing`
eller `openai_quota`); endast en PR vars live REST-author är exakt
`dependabot[bot]`/`Bot` får använda kontolänen utan request, eftersom den primära
workflowen avsiktligt skippar Dependabot. Därefter används ett separat
tvåresursbevis: SHA-bunden review plus PR-kommentar med review-ID. Båda måste
komma från samma konfigurerade, betrodda repository-actor och exakt live head.
