...
- **Guest generation limit:** Without login, generation is allowed once per session cookie (`guest_usage` table: `generate` limit 1, `refine` limit 1, tracked by `sessionId`). After that the builder shows a credits/login modal. Clear cookies / use a fresh Incognito window to get a new free generation. `/api/ai/brief` (client-triggered Deep Brief) intentionally returns 401 for guest/anonymous users — that is by design and is soft (server auto-brief covers create-chat), not a misconfiguration.
