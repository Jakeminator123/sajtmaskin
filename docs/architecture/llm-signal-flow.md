...
| **Capability-inferens (init)** | `capability-inference.ts` | Unicode-aware regexar + manifest | `buildDynamicContext()`, `BuildSpec`, `follow-up-clarification` | Nej. Follow-up dossier-mapning sker parallellt via `follow-up-capability-detection.ts` + `follow-up-capability-vocabulary.ts` (annan tröskel än `needs3D`/`needsGame` här) — se regressionsmatris i `follow-up-clarification.test.ts`. `svävande`/`hovrande`/`flygande` ensamt är motion, inte `visual-3d`; explicit `3D`/`WebGL`/`mesh`-ord krävs för 3D-dossier. |
...
| **Per-run phase model trace** | `src/lib/models/phase-routing.ts` → `buildOwnEngineGenerationStreamMeta().phaseModelTrace` | builder model-trace/debug surfaces, stream-meta/logginspektion, framtida backoffice-inspektion | Planner / generator / fixer / verifier / deploy-assistant med konkret `modelId`, `thinking`, `reasoningEffort` för den valda build-tiern |
...
6. Dynamic context byggs i `src/lib/gen/system-prompt/`. När briefen bär designvärden renderas `## Brief-Locked Design Values` före `## Scaffold Variant (this generation)` och med högre pruning-prioritet, så variantens tema/font/motif bara är fallback när briefen är tyst. `## Your Toolkit` byggs från registry-synkade `SHADCN_COMPONENTS`-mappen, filtrerad mot vilka `@/components/ui/*`-subpaths som faktiskt finns lokalt; `## UI Recipes` lägger separat till capability-matchade shadcn registry-recept från `shadcn-ui-recipes.ts`. I `simpleWebsitePath` är UI Recipes avstängda för att hålla init-konteksten kort.
