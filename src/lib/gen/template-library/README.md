...
2. **Variant structural files (opt-in):** When `SAJTMASKIN_VARIANT_STRUCTURAL_FILES=true`, two passes inject structural code excerpts into `## Structural References (this variant)`: (a) variant-driven — reads the active scaffold variant's `sourceTemplateIds` and picks up to 3 files, (b) capability-driven — searches the full catalog for entries matching `InferredCapabilities` signals (auth, ecommerce, dashboard) not already covered by the variant pass and picks up to 2 additional files. Only init generations and `isFirstCodeGeneration` are affected; normal follow-ups skip this.
