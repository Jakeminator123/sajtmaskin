...
- **Ägarprioritet preview + mall-distinktion (2026-03-25):** **K-018** ny (användarsidor: React som `npm run dev`, fidelity i **`iframe`**); **K-008 [x]** (landning fryst); **K-009** scope i `own-engine-sse-scope.md` (**[x]/[N/A]** 2026-03-26); FAQ + `e2e/README` — **Vercel-templates = scaffolds** för OwnEngine, **V0-templates** separat; `PLAN-KRITIK-OPEN`, `COMPLETION-ROADMAP`, `KORFIL`, `REMAINING-WORK`, `kritik-consolidated`, progress, `ORCHESTRATOR_LOG`.
...
- **W2 / deploy-kontrakt (2026-03-26, K-007 delmoment → K-007 [x] 2026-03-26):** Vitest för **`precheckOnly`** utan `skipAutoFix` (auto-fix körs) **och** `precheckOnly` + `skipAutoFix`; `docs/architecture/deploy-precheck.md` § **Kontraktstester** + § *Produktbeslut K-007*.
