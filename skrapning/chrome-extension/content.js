...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'extractTemplates') {
    ...
    console.log('Extracted templates:', templates.length, 'Category:', currentCategory);
...
function extractTemplatesFromDOM() {
  ...
  links.forEach((link) => {
    ...
    if (!href || href === '/templates' || href === '/templates/' || href.includes('submissions')) {
      return;
    }

    // Skip category pages like /templates/landing-pages (but not individual templates)
    const categoryPages = ['landing-pages', 'hero-sections', 'forms', 'dashboards', 'ecommerce', 'pricing', 'features'];
