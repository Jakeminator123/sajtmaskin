...
- If a test fails, investigate — it is usually a real regression, not a pre-existing environment issue. There are two known categories of expected failures in the Cloud Agent env:
  - **Platform-injected-secret failures (3, not regressions):** `npm run test:ci` reports 3 failures (`src/lib/kostnadsfri/index.test.ts`, `src/app/api/domains/link/route.test.ts`, `src/app/api/kostnadsfri/[slug]/verify/route.test.ts`) caused purely by platform-injected secrets (`VERCEL_PROJECT_ID`, `VERCEL_TEAM_ID`, `KOSTNADSFRI_API_KEY`, `KOSTNADSFRI_PASSWORD_SEED`) — these negative-path tests assume those vars are unset. They pass when those vars are unset (e.g. `env -u VERCEL_PROJECT_ID -u VERCEL_TEAM_ID -u KOSTNADSFRI_API_KEY -u KOSTNADSFRI_PASSWORD_SEED npx vitest run <files>`).
  - **Pre-existing PreviewPanel save-flow failures (2):** `src/components/builder/PreviewPanel.test.tsx` has 2 failing tests — "saves blog post editor changes through the files PATCH route" and "saves footer editor changes through the files PATCH route" (the editor *renders* fine; only the save→PATCH→`onFilesSaved` flow times out in `waitFor`). These reproduce consistently in isolation and on the pinned Node 22.22.2, are independent of env vars / secrets / DB, and are present as of HEAD `7706a27` (2026-06-27). Treat as a pre-existing repo issue, not something you broke — do not "fix" it as part of unrelated work.
