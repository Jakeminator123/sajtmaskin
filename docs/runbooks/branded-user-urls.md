...
Kör inte dessa steg förrän A1 har två testhosts med giltig HTTPS. Inga
Vercel-alias, inga one.com-poster och inga `sajtmaskin.se`/`.com`-testdomäner
...
1. DNS+alias för `pilot-a1-test.sites.sajtmaskin.se` → tesprojekt A.
2. DNS+alias för `pilot-a1-peer.sites.sajtmaskin.se` → tesprojekt B
   (A1 har inte namngett peer-hosten; detta är det exakta värdnamn A3/A4
   behöver utöver `pilot-a1-test`).
...
1. **Branded.** Publicera den granskade versionen. Förvänta
   `NEXT_PUBLIC_SITE_URL=https://pilot-a1-test.sites.sajtmaskin.se`,
...
Tesprojekt B (`pilot-a1-peer`) används bara för att bevisa att A:s redirect
