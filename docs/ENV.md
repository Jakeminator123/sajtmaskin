...
**Ärlig publiceringsgrind (deploy-409):** `POST /api/v0/deployments` blockerar med `409 DEPLOY_MISSING_ENV` på `buildBlockingKeys` i F3 (`integrations`) och på `missingEnvKeys` (legacy-backstop, "helt frånvarande" nycklar) i F2 (`design`) — F2 förblir alltså demo-publicerbart. `feature-runtime`/placeholder-täckta nycklar hård-blockerar aldrig; de surfar som icke-blockerande `EnvDegradationWarning` (`src/app/api/v0/deployments/env-degradation-warnings.ts`). F3-stream/finalize-design gatar dessutom på samma riktiga build-nycklar (`412 tier3_env_not_ready`) och pekar mot Dossiers-panelen, inte chatten.
