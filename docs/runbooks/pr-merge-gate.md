...
Den GitHub-integrerade Bugbot:en delar teamets budget och postar `Bugbot couldn't run - usage limit reached` när den tar slut. Lokal Sol/bugbot är då innehållsfallback. Den required `review-window` blir dock bara grön när minst ett maskinverifierat reviewkvitto för aktuell head lyckas — normalt `trusted-pr-ai-review`, den tvåresursverifierade kontobaserade Codex-fallbacken eller en extern reviewbot. Det betrodda kvittot får `success` först efter en uttömmande review av exakt live-headens hela diff. API-billing-/kvotspärr lämnar ett neutralt överlämningskvitto; skip, fyndspecifik uppföljning, stale resultat och en vanlig kontokommentar räknas inte som pass. Är alla kvalificerande kvitton borta/röda stoppas merge i stället för att timeout bli falskt grön.
