...
function MinimizedButton({
  ...
  return (
    <motion.button
      ...
      title={disabled ? "Laddar..." : "Visa guiden"}
    >
      <MessageCircle
      ...
      />
      {/* Pulse effect - only when not disabled */}
      {!disabled && (
...
export function FloatingAvatar({
  ...
  const {
    ...
    hideAvatar,
    showAvatar,
    // Minimized state now persists across page navigation
    isMinimized,
    setMinimized,
  } = useAvatar();
  ...
  // IMPORTANT: Disable clicks while 3D model is loading to prevent issues
  const handleAvatarClick = useCallback(() => {
    // Prevent clicks while model is loading
    if (avatarState === "loading" || !isLoaded || !canvasReadyRef.current) {
      return;
    }
  ...
  }, [chatOpen, hideTooltip, avatarState, isLoaded]);
  ...
  // Disable button while model is loading
  if (isMinimized || avatarState === "hidden") {
    const isLoading =
      avatarState === "loading" || !isLoaded || !canvasReadyRef.current;
    return (
      <AnimatePresence>
        <MinimizedButton onClick={handleRestore} disabled={isLoading} />
      </AnimatePresence>
    );
  }
    ...
    >
      <AnimatePresence>
        ...
        >
          ...
          {/* Avatar container */}
          {/* IMPORTANT: Disable pointer events and cursor while loading */}
          <div
            className={`relative ${AVATAR_CONFIG.containerWidth} ${
              AVATAR_CONFIG.containerHeight
            } group ${
              avatarState === "loading" || !isLoaded || !canvasReady
