...
| OpenClaw edit-agent (opt-in) | `OPENCLAW_EDIT_AGENT` | **Opt-in, default av.** `OPENCLAW_EDIT_AGENT=true` låter OpenClaw editera användarprojektet från en prompt: `POST /api/openclaw/edit` tolkar prompten till deterministiska quick-edit-ops (via gatewayen), applicerar dem på senaste versionen med `runQuickEdit` och patchar preview-VM:en. **Av = routen 404:ar** och widgeten döljer redigeringsknappen (dagens "föreslå prompt" gäller). Kräver att OpenClaw-ytan i övrigt är på (`OPENCLAW_GATEWAY_URL/TOKEN` + `IMPLEMENT_UNDERSCORE_CLAW`). Test-/preview-flagga — sätt inte i production utan avsikt. Läses via `OPENCLAW.editAgentEnabled` i `src/lib/config.ts`. |
