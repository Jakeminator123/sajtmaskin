...
/**
 * Opt-in Pac-Man cookie game ("Cookie Quest").
 *
 * Rendered as a fullscreen modal ONLY when the user actively chooses to play
 * from the compact cookie banner — it is never the default consent surface.
 * Winning the game counts as accepting cookies. Loaded lazily (dynamic import
 * from `cookie-banner.tsx`) so the game code never ships in the initial bundle.
 */
...
interface CookieGameModalProps {
  /** Called when the user wins the game (= accepts cookies). */
  onWin: () => void;
  /** Called when the user closes the game without finishing. */
  onClose: () => void;
}

export function CookieGameModal({ onWin, onClose }: CookieGameModalProps) {
  ...
  }, [movePacman, gameStarted, onClose]);
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label="Cookie Quest — spela för att acceptera cookies"
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm"
    >
      <div
        className="relative border-2 border-[#1a1aff] bg-[#0a0a0a] p-6 shadow-[0_0_60px_rgba(26,26,255,0.3)]"
      ...
      >
        ...
        >
            ...
            row.map((cell, x) => {
              ...
              return (
                ...
                >
                  ...
                  {isCookie && (
                    <div
                      className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 animate-pulse text-sm motion-reduce:animate-none"
                      style={{
          ...
          {!gameStarted && (
            <div className="absolute inset-0 flex items-center justify-center bg-black/60">
              <div className="animate-pulse text-center motion-reduce:animate-none">
          ...
          {hasWon && (
            <div className="absolute inset-0 flex items-center justify-center bg-black/70">
              <div className="text-center">
                ...
                <p className="text-brand-amber animate-pulse text-[9px] motion-reduce:animate-none">
                  WAKA WAKA!
                </p>
        ...
        <p className="mt-3 text-center text-[8px] text-gray-500">
          Nå kakan 🍪 för att acceptera cookies — eller stäng med Esc
        </p>
      </div>
    </div>
  );
