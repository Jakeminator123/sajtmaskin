...
| F2 render-first advisory | I F2 (`design`) failar ett typecheck-*only*-fel inte versionen. Regelägare: `isTypecheckOnlyAdvisory()` (`quality-gate-checks.ts`), delad av `quality-gate`-routen och bakgrunds-`server-verify` så vägarna aldrig är oense. Routen promotar och svarar `{ passed: true, vmGatePassed: false, designAdvisory: true, advisoryChecks: ["typecheck"] }` (ingen auto-repair i `post-checks.ts`); server-verify försöker promota FÖRE terminal-emit och emitterar ingen terminal bus-händelse vid promote-no-op. `next dev` renderar JS trots typfel. Build/lint/verifier/promote-guard förblir hårda; F3 kör alltid `typecheck+build+lint` hårt. Render-säkerheten ägs uppströms av finalize-preflight. |
...
| Dossier F2/F3-gräns | Samma dossier kan spänna F2 och F3. F2 renderar en klient-/demo-/placeholder-safe version; F3 aktiverar riktig integration. Kanonisk signal i dagens kod: dossierns `envVars`/`enforcement: "build"` (helper `dossierRequiresF3`, uppräknad via `getF3RequiredCapabilities`). Ingen separat `hard/soft/visual`-taxonomi styr fasen; `envVars: []` ⇒ fullt F2-användbar. |
