...
|---|---|---|---|---|---|
| Prompt formatting | minimal fallback-wrap (`MÅL` + `TILLGÄNGLIGHET`) i prompt-wizard, inte på create-chat-vägen | `src/lib/builder/prompt-assist/formatters.ts` | rå användarprompt | formatterad prompt | torftig prompt förblir för lös, för lite domänstruktur |
...
| Scaffold keyword match | domänord för auth/ecommerce/blog/portfolio/website/app + brief-context | `src/lib/gen/scaffolds/matcher.ts`, `orchestrate/scaffold-query-context.ts` | rå prompt + Brief (`pages`, `styleKeywords`, `domainProfile`) | scaffold-id + keyword scores | `domainProfile` mappas till bank-token; `Math.max` mot prompt-score — ingen platt +2. `toneAndVoice` styr copy/variant, inte scaffold-eligibility. `SAJTMASKIN_SCAFFOLD_KEYWORD_MATCH=off` stänger keyword-valet |
...
| Källkvitto | vilka valda källor som överlevde tokenbudgeten och, för variantbilden, visionkanalens tak | `src/lib/gen/orchestrate/source-receipt.ts`, `finalize-prompts.ts`, `persist-telemetry.ts` | variantreferens, UI Recipes, dossiers, `mediaCatalog`, designreferenser + `keptBlockKeys` + användarbilagor via `getVisualReferenceAttachments` | `sources[]` på paketet, dump-meta (`sourceCount`/`sourceKinds`) och `generation_telemetry.meta.sources`. Variantreferensens `reachedPrompt` är sant om textblocket överlevde **eller** stillbilden fanns i den skickade visionpayloaden (fyra bilder, användarbilder först) — och förblir en OR, eftersom kanalerna är oberoende. För att OR:en inte ska dölja vad som faktiskt levererades bär `variant-reference` dessutom tre valfria signaler: `stillImageSent` (bilden låg i den skickade payloaden), `inspirationBlockKept` (textblocket överlevde budgeten) och `addendumTextSent` (blocket överlevde **och** posten hade utdrag). `addendumPruned` och `noUsableAddendum` härleds ur dem plus `reason` (`addendum:${state}`) och har inga egna fält. Ett behållet block är inte samma sak som levererade utdrag: en `disabled` eller utdragslös post renderar ändå blocket. Övriga `kind` bär inte flaggorna. | media-katalogen är tom tills en anropare skickar `mediaCatalog`; designreferenser kvitteras ändå. Ändrar inte urval. Fyra användarbilder tränger ut variantbilden — kvittot ska då inte säga att bilden nådde prompten. |
...
Briefen matar in i scaffoldmatchningen via `ScaffoldQueryContext` (`pages`, `styleKeywords`, `domainProfile`→`domainHints`). `toneAndVoice` når i stället copy och scaffold-variant-resolvern; ton ensam får inte göra en domänscaffold valbar via keyword eller embedding. `businessType`/`industry` läses inte — de finns inte i `siteBriefSchema`. Route-planen mergear brief-routes som startpunkt, inte override. Se [`docs/contracts/component-library.md`](../contracts/component-library.md) för komponentbibliotekspolicy.

**Ingen längdgräns och ingen hemsida/app-skillnad (B8, 2026-08-18).** Fram till dess tog korta hemsideprompter (≤ 420 tecken) ett snabbspår — `simpleWebsitePath` — som hoppade över Auto Brief, scaffold-embeddings, UI Recipes och dossier-selektion. Appar avvisades alltid från spåret och fick därför hela vägen. Spåret är borttaget: ett fritextbygge får samma berikning oavsett promptlängd och oavsett `buildIntent`. Init-Auto Brief väntas med en egen 70 s-budget (`SERVER_AUTO_BRIEF_TIMEOUT_MS`); timeout hoppar över briefen och startar codegen ändå. Rollback vid latens-/kostnadsregression sker med `SAJTMASKIN_DISABLE_SERVER_AUTO_BRIEF=1` (Vercel-targets: development/preview/production) respektive `SAJTMASKIN_DOSSIER_PIPELINE=false`, inte genom att återinföra en längdgräns. Verbatim-import (`POST /api/template`) är en egen väg och berörs inte.
