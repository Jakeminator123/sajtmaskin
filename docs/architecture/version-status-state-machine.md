...
1. **Resume av strandad verify-lane:** `useResumePendingVerification` (monterad i `useBuilderPageController`) återupptar verifieringen för chattens senaste version när den är `draft`+`pending`, F2 (ej `integrations`), har `edit_kind = NULL` (provenance-gate: `quick_edit`/`imported_repo`/`restore`-rader hade aldrig någon browser-lane och ska varken promotas eller false-red:as av en resume) och är 3 min–24 h gammal (`RESUME_VERIFY_MIN_AGE_MS`/`RESUME_VERIFY_MAX_AGE_MS`). Resumen speglar ordinarie lane-svans (Codex round 1+2 på #353): `POST /validate-images` (auto-ersätter trasiga bild-URL:er) → `POST /product-postcheck` (emitterar `version.degraded` server-side OCH persisterar resultatet till `/error-log`, inkl. `product_postcheck.summary`-raden som `PreviewPanelF3Trigger` läser för F3-blocket) → `POST /quality-gate`. `productBlocked` stoppar INTE gaten — ordinarie lane loggar det som warning och verifierar ändå, så raden settlar (promoted-med-degradation eller failed) i stället för att bli hängande `pending`; F3-lyftet blockeras via den persistade summary-raden. En stängd/omladdad flik lämnar alltså inte längre versionen ogrön för alltid; nästa builder-besök plockar upp den. Routens per-version-lease gör dubbelkörning ofarlig (409 `version_busy`). Provenance-stödet: restore/rollback-drafts markeras nu `edit_kind="restore"` och ZIP/GitHub-import (`/api/engine/chats/init`) markeras `edit_kind="imported_repo"` (samma marker som template-import, vilket också ger imported-repo-preflightrelaxen på follow-ups).
