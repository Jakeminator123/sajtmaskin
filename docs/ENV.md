...
| Pre-VM eslint | `SAJTMASKIN_BLOCKING_ESLINT=true`, ev. `SAJTMASKIN_BLOCKING_ESLINT_MAX_WARNINGS=20` | Aktiverar `eslint` mot samma warm-cache efter att warm-tsc passerar. **Obs:** F2:s VM-gate + bakgrunds-server-verify kör bara typecheck (designPreview-lane), så lint i F2 körs *enbart* via detta warm-pass — och bara när flaggan är på (eller F3-force) **och** cachen är varm för vald scaffold; annars skippas lint fail-open (`cache_cold`). Utfall syns i `site.done` → `warmEslint`. Samma cache-rot som warm-typecheck (`SAJTMASKIN_PRE_VM_TYPECHECK_CACHE_ROOT`). Källa: `src/lib/gen/preview/warm-eslint.ts`. Provisioneras via `npm run provision:warm-cache` (se `scripts/provision-warm-cache.ts`). |
...
**Dossier-scopad:** `env.example` dumpar inte längre hela placeholder-katalogerna. Preflight skickar in `selectedDossiers` env-nycklar (`dossierEnvScope`) i `injectProjectEnvFileIntoFilesJson` → `buildProjectEnvFileContents`, så filen bara listar (a) användarens sparade värden, (b) nycklar modellen själv skrev i `.env.local`, (c) projekt-preview-tokens och (d) env-nycklar för de **valda** dossiers — med placeholder-värden från 40/41-katalogerna när nyckeln finns där, annars en tom `KEY=`-rad med `purpose`-kommentar. En sajt utan dossiers får alltså ingen katalogdump. Det F2-only "Upptäckta integrationer (kommenterade)"-blocket (redan projekt-scopat) behålls. Utan scope (äldre/okända callsites) faller det tillbaka på full dump.

| Stage | Innehåll i `env.example` | Källor |
|-------|---------------------|--------|
| **F2** (`design`) | Projekt-preview-tokens + användar-/genererade lager **+** env-nycklar för valda dossiers (harmless/tier-3-placeholders där de finns, annars tom rad med `purpose`). Användaren ser exakt vilka nycklar just den här sajten kan använda. Ingen interaktion krävs — preview-VM:en bootar oberoende av denna fil. | `dossierEnvScope` (valda dossiers) matchat mot `40-harmless-placeholders.env.txt` + `41-tier3-stub-placeholders.env.txt` + `project-preview-env.ts` |
| **F3** (`integrations`) | Tier-3-stubs strippas. Värden från env-panelen (`projectEnvVars` i DB) mergas in som "user"-lager. Dossier-nycklar utan riktigt värde surfar som tomma rader; saknade tier-3 nycklar blockeras via [`src/lib/integrations/tier3-build-spec.ts`](../src/lib/integrations/tier3-build-spec.ts). | Som F2 utan tier-3 + DB-lagrade `projectEnvVars` + ev. modell-emitterad `.env.local` |
