...
Visuell karta över FK-cascade-kedjan i Postgres efter migrationerna
...
| `pricing_settings`                                    | Singleton (`id = 'default'`) med operatörsstyrt domänpåslag och de fasta creditpriserna. Egen tabell, inte kolumner på `generation_billing_settings`: den ägs av LLM-avräkningen och fryses per generering. Ingen FK — raden beskriver produkten, inte ett projekt.          |
