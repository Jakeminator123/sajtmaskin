 ...
 *
 * This is intentionally a real HTTP check and therefore belongs to the
 * warn-only stability lane. External DNS, host availability and rate limits
 * must never make the deterministic `test:ci` merge gate flaky.
 */
...
describe("tailark-oss live connectivity", () => {
  it("reaches seeded Mist items over the configured Radix OSS URL", async () => {
  ...
  it("does not treat the gated Quartz host as reachable without auth", async () => {
