...
| v0/engine route consolidation (P29) | 2026-04-20: hela `/api/v0/chats/**`-trädet borttaget. Fas 1A (18 testlösa re-exports) + Fas 1B (10 routes vars unique test-coverage migrerades till engine-side test-filer först) levererade. `src/lib/api/engine/chats/v0-chats-compat.ts` (`logLegacyV0ChatsHit`) borta — inga callers kvar. Audit `03-konsolidering-pipeline.md` §3.4 / Tier A #14 stängd för chat-ytan. Kvar: 7 Class C legacy-routes (`init-registry`, `integrations/vercel/projects`, `projects/instructions`, `projects/[id]/env-vars`, `deployments/*`) är legitim legacy med faktiska klient-callsites — väntar på Fas 2-beslut (rename till `/api/legacy/v0/*` eller behåll). Klient-prefixen för aktiv chat-trafik är `ENGINE_CHATS_API_PREFIX` i `src/lib/api/engine-chats-path.ts`. | kanonisk |
