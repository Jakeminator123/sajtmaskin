...
- **shadcn- och strukturreferenserna är nu flerskiktade:** `## Your Toolkit` byggs i `system-prompt.ts` från den registry-synkade `SHADCN_COMPONENTS`-mappen, men begränsas till subpaths som faktiskt finns lokalt under `@/components/ui/*`; `config/prompt-static/03b-shadcn-component-patterns.md` ger alltid-på usage-regler; `## Component References` lägger till capability-matchade kodexempel från `data/shadcn-examples/`; och `## Structural References (this variant)` kan, när `SAJTMASKIN_VARIANT_STRUCTURAL_FILES=true`, lägga till budgeterade `layout.tsx`-/`page.tsx`-/`middleware.ts`-utdrag från variantens `sourceTemplateIds` i template-library-katalogen. Request-specifik shadcn-/registry-/palette-/variantkontext kan därefter utöka både komponentvokabulär och strukturella startmönster när hosten redan skickat sådan payload.
