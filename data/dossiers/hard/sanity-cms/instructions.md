...
- Do not put `SANITY_API_TOKEN` in a `NEXT_PUBLIC_*` variable, and never import `@/lib/sanity/token` or `@/lib/sanity/client` from a client component — both are `server-only`.
...
- Set `SANITY_API_TOKEN`, open the configured Presentation Tool preview URL, and confirm draft mode enables and draft content appears only in draft mode.
