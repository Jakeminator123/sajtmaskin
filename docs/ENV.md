...
| shadcn "Beskriv"-discovery (opt-in) | `NEXT_PUBLIC_SAJTMASKIN_SHADCN_DESCRIBE` | **Opt-in, default av.** Slår på Fas 1-routen `POST /api/shadcn/describe`: fritext-beskrivning → LLM skriver 1–3 registry-sökfrågor → sök över officiella + community-register (HTTP mot `registry-service`) → LLM rankar 5–10 **verkliga** träffar → kandidater `{name, registry, description, previewLight/Dark, dependencies, registryDependencies, addCommand}`. **Skriver inget till användarsajten.** Flagga av → routen svarar `404`, ingen ny kodväg (noll beteendeändring). Återanvänder befintliga `OPENAI_API_KEY`/`ANTHROPIC_API_KEY` via `src/lib/models/*` — ingen ny hemlighet; utan nyckel faller query/rankning tillbaka på deterministisk heuristik. Källa: `src/lib/shadcn/describe.ts`, `src/lib/shadcn/describe-feature.ts`. Plan: `docs/plans/active/2026-07-22-shadcn-registry-beskriv-komposition.md` (Fas 1). |
| Add-panel / Bläddra (opt-in) | `NEXT_PUBLIC_SAJTMASKIN_ADD_PANEL` | **Opt-in, default av.** Slår på den tabbade "Lägg till"-ytan i buildern (flikar **Block** / **Bläddra** / **Beskriv**). "Block" = dagens Composer-drag-and-drop oförändrad; "Bläddra" väcker shadcn-registry-galleriet (kort + thumbnails via `/api/shadcn/registry`-proxyroutesen); "Beskriv" är en disabled platshållare. **Ingen insättning kopplad ännu** (Fas 3 — kortval öppnar bara detaljvy). Flagga av → dagens fristående "Composer"-knapp/palette, ingen registry-fetch. Källa: `src/lib/builder/add-panel-feature.ts`, `src/components/builder/preview-panel/PreviewPanelAddPanel.tsx`. Plan: `docs/plans/active/2026-07-22-shadcn-registry-beskriv-komposition.md` (Fas 3). |
