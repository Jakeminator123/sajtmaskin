...
## Postamble — 2026-04-21 follow-up

`deduplicateLocaleAlternateRoutes` levererades enligt steg 4 men **anropades aldrig från `buildRoutePlan()`** — den var deklarerad och testad men dead code i produktionsflödet. Detta upptäcktes 2026-04-21 efter att en blogg-generering producerade både `/blogg` (från brief) och `/blog` (från prompt-pattern + scaffold-default), vilket fick LLM:n att skriva `href="/blog/${slug}"` mot en literal `/blog/slug`-route → trasig navigation. Åtgärdat 2026-04-21:

- `buildRoutePlan()` kör nu `dedupePlannedRoutesInPlaceByLocale()` direkt efter explicit-page-count-trim (med `locale`-param, default `sv`).
- `/blog ↔ /blogg` lades till i `LOCALE_ROUTE_PAIRS`.
- Ny deterministisk preflight-check `crossCheckHrefsAgainstRoutes()` i `src/lib/gen/verify/href-route-cross-check.ts` fångar samma klass av fel även när dedupen missar (severity warning initialt).
- System-prompten får en *Canonical route paths*-sektion som listar exakta tillåtna paths.

Lärdom: `validator_hooks` ska inte bara verifiera att en symbol *finns* i en fil — den ska verifiera att den *används* från sin förväntade callsite. Plan för fas-2-cleanup: `docs/plans/active/repair-loop-hardening.md`.
