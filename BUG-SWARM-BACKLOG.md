...
| [ ] | Bekräftad prod-bugg | P1 | `SM-072` Chromium-captures svälter `/tmp` på warm Fluid-instans: burst av postcheck/live review läcker tills nästa launch dör (`Target page, context or browser has been closed`) och versionen visas som Degraderad trots frisk sajt. | Vercel runtime 2026-08-31 (chat `30840b09`, dpl `dpl_81XhZUJJ…`): 513→31→23 MB fritt inom 5 min; DB-skip `playwright_unavailable` + `runtime_error`; plattformsbred defektsignatur `e18935fd85a9` (6 chattar sedan 22 aug). Utredning: `docs/plans/active/2026-09-01-verifieringsflode-och-inspector/`. | ÄTAREN NAMNGIVEN i prod 2026-09-01 (chat `3b9ca137`): topplistan visade `core.chromium.24=913MB` — Chromiums core dumps fyller tmpfs:en ensamma. Fix i PR `fix/resume-verify-double-postcheck`: (a) `core.chromium.*` raderas oavsett ålder före varje serverless-launch, (b) klientlanen kör om postchecken EN gång vid infrastruktur-skip (omkörningen fångade 6 riktiga produktfynd som första försöket missade), (c) resume-lanen krockar inte längre med en levande normal lane (vakt + åldersgräns 3→5 min). Verifiera i prod-logg efter merge: `pruned N Chromium core dump(s)` + inga `playwright_unavailable` i följd. |
