...
/** Delad drag-och-släpp-lista över Composer-blocken. Delas av dagens fristående
 *  palette och den inbäddade "Block"-fliken i "Lägg till"-ytan (flag-gated). */
function ComposerBlockList(props: {
  disabled?: boolean;
  onDragStart?: () => void;
  onDragEnd?: () => void;
}) {
...
export function PreviewPanelComposerPalette(props: {
  ...
  onDragEnd?: () => void;
  /**
   * När true: renderas utan egen `<aside>`-chrome/header (för inbäddning i
   * "Lägg till"-ytans "Block"-flik). Default false = exakt dagens fristående
   * Composer-palette (flag-av-vägen är byte-för-byte oförändrad).
   */
  embedded?: boolean;
}) {
  const { disabled, onDragStart, onDragEnd, embedded } = props;
  ...
  if (embedded) {
    return (
      ...
      >
        ...
        <ComposerBlockList disabled={disabled} onDragStart={onDragStart} onDragEnd={onDragEnd} />
      </div>
    );
  }
