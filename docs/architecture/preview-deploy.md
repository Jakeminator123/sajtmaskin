...
**Tier-2 preview `.env.local`:** Både **`startPreviewSession`** (builder/UI) och **`generateOwnEngineSiteFromPrompt`** (MCP/own-engine) anropar `buildPreviewEnvLocalContents` (`src/lib/gen/sandbox/env-local.ts`) som bygger merged `.env.local` i VM — globala placeholders från `config/ai_models/40-generated-site-integration-placeholders.env.txt`, projekt-preview-token, lagrade projekt-env, sist genererad `.env.local` om modellen skrev en (senare vinner). Se `config/user_degraded_env.txt` och avsnittet *Genererade användarsajter* i [`docs/ENV.md`](../ENV.md).
