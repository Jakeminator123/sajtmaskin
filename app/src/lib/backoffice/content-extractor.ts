...
const PATTERNS = {
  // Text in JSX
  jsxText: /<(?:h[1-6]|p|span|button|a|label|li)[^>]*>([^<]+)<\//g,

  // String literals (for dynamic content)
  stringLiteral:
    /(?:title|label|text|heading|description|placeholder|alt)\s*[:=]\s*["']([^"']+)["']/gi,

  // Image sources
  imageSrc: /(?:src|image|img|photo|picture)\s*[:=]\s*["']([^"']+)["']/gi,

  // Tailwind color classes
  tailwindBg: /bg-(\w+)-(\d+)/g,
  tailwindText: /text-(\w+)-(\d+)/g,

  // Hex colors
  hexColor: /#([0-9a-fA-F]{3,8})/g,

  // Price patterns
  price: /(\$|€|kr|SEK)\s*[\d,]+\.?\d*/gi,

  // Product-like structures
  productArray: /(?:products|items|menu|cards)\s*[:=]\s*\[/i,
};
