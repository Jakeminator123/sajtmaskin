...
export function usePromptAssist(params: UsePromptAssistParams) {
  ...
  const maybeEnhanceInitialPrompt = useCallback(
    async (originalPrompt: string): Promise<string> => {
      ...
      try {
        toast.loading('Förbättrar prompt (AI Assist)...', { id: 'sajtmaskin:prompt-assist' });
        ...
        const betterMessage = (() => {
          ...
          if (provider === 'vercel') {
            return (
              'Kunde inte nå v0 Model API. Kontrollera att V0_API_KEY eller VERCEL_API_KEY är satt.'
            );
          }
          if (provider === 'openai') {
