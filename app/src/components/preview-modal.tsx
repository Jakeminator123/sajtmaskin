...
export function PreviewModal({
  ...
  useEffect(() => {
    const handleEscape = (e: KeyboardEvent) => {
      // Only close if Escape is pressed and not typing in an input/textarea
      if (e.key === "Escape") {
        ...
        // Check if focus is inside iframe (can't reliably detect, but try)
        // If iframe has focus, we still want to close the modal
        e.preventDefault();
        e.stopPropagation();
        onClose();
      }
    ...
    if (isOpen) {
      // Use capture phase to handle before iframe can intercept
