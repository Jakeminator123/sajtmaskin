...
| Preview prewarm (levels 1+2) | `SAJTMASKIN_PREVIEW_PREWARM=true` | **Opt-in.** When `true`/`1`: the first generation of a NEW chat fires a fire-and-forget preview-host boot with the baseline scaffold skeleton at generation start (init flow: `create-chat-stream-post.ts`; versionless first-gen: `chat-message-stream-post.ts`), so the preview host wakes (level 1) and the install runs on the VM while the LLM is still streaming (level 2). When the finalize `package.json`/lockfiles match the baseline (common case) the finalize boot reuses `node_modules` and **skips install** (fingerprint match in `preview-host/src/runtime.js`); otherwise install still runs at finalize. Requires a tier-2 preview host (`SAJTMASKIN_PREVIEW_HOST_BASE_URL`) configured. Follow-ups are never prewarmed. **Default off** - best-effort; MEASURE the net win on the host before enabling. Source: `src/lib/gen/preview/preview-prewarm.ts` (`FEATURES.previewPrewarm`). |
...
| **F3** (`integrations`) | Tier-3-stubs strippas. Värden från env-panelen (`projectEnvVars` i DB) mergas in som "user"-lager. Dossier-nycklar utan riktigt värde surfar som tomma rader; saknade `build`-nycklar blockeras via [`src/lib/integrations/tier3-build-spec.ts`](../src/lib/integrations/tier3-build-spec.ts). | Som F2 utan tier-3 + DB-lagrade `projectEnvVars` + ev. modell-emitterad `.env.local` |
