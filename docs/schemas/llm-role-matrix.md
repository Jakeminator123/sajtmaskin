...
| **Delta brief** | Bara vid `followUpIntent === "clear-redesign"`. Samma `siteBriefSchema` som init (`Include every field`), med `formatPriorDesignContext(..., { intent: "clear-redesign" })`. Inte en smal diff. Övriga uppföljningar får Snapshot-Brief, inte `null`. | Skriver tillbaka brief för den follow-up-rundan. | `runClearRedesignDeltaBriefPhase` |
