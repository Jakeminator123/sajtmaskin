...
| `POST /api/github/export` | Engine-chat: filer från `files_json`; annars legacy V0 `getVersion` | Legacy-grenen gatead bakom `SAJTMASKIN_V0_PLATFORM_ENABLED` |
...
| `POST /api/v0/chats/init` | Returnerar **410** när `SAJTMASKIN_V0_PLATFORM_ENABLED` är av; migrera till own-engine | Teknisk skuld |
| `POST /api/template` | Returnerar **410** när `SAJTMASKIN_V0_PLATFORM_ENABLED` är av; migrera till own-engine | Teknisk skuld |
