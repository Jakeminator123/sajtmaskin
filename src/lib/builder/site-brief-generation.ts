...
export const siteBriefSchema = z.object({
  ...
  requestedCapabilities: z
    ...
    .max(12)
    .describe(
      "Real requested capability ids in kebab-case, e.g. auth, payments, subscriptions, database, cms, realtime, image-generation, ai-chat, ai-tool-calling, rag-chat, visual-3d, physics-3d, carousel, command-palette, site-search, map-display, gallery-lightbox, dashboard-charts, contact-form, newsletter-subscribe. There is ONE auth capability (`auth`) — the provider (Clerk default / Supabase on explicit ask) resolves downstream. Plain content sections (FAQ, pricing, testimonials, logo row, stats, CTA, multi-step form UI) are page content, not capability ids. Do not list generic page sections like hero, about, contact, footer, or SEO.",
    ),
  pages: z
    .array(
      z.object({
...
const BRIEF_SYSTEM_PROMPT =
  ...
  "- `requestedCapabilities` is for real implementation capabilities only. Prefer existing ids when relevant: `auth`, `payments`, `subscriptions`, `database`, `cms`, `realtime`, `image-generation`, `ai-chat`, `ai-tool-calling`, `rag-chat`, `visual-3d`, `physics-3d`, `carousel`, `command-palette`, `site-search`, `map-display`, `gallery-lightbox`, `dashboard-charts`, `contact-form`, `newsletter-subscribe`. The integration ids are explicit-ask-only: add `subscriptions` ONLY when the user explicitly asks for RECURRING subscriptions/memberships (Paddle Billing, återkommande betalning, medlemskap, prenumerationsplan) — a one-off card purchase/checkout is `payments`, not `subscriptions`, and a newsletter signup is `newsletter-subscribe`; add `database` ONLY when the user explicitly asks for persistent server-side data storage (saving orders/bookings/submissions to a database, a data backend, or names Postgres/PostgreSQL/MongoDB/Neon) — NOT for static content pages, NOT for a contact form alone (that is `contact-form`), and NOT for vector/semantic search stores; add `cms` ONLY when the user explicitly asks for a content management system or editing content without code (names a CMS/headless CMS like Sanity, or asks that editors/staff can update posts/pages/products themselves) — NOT for ordinary static pages, NOT because the site merely HAS a blog or news section, and NOT for site-builder/admin features; add `realtime` ONLY when the user explicitly asks for realtime infrastructure (live chat between visitors, presence, live notifications, collaborative/multiplayer updates) — NOT for 'live-feeling' animations or real-time analytics dashboards; add `image-generation` ONLY when the user explicitly asks the SITE ITSELF to generate images with AI (text-to-image tool) — NOT for image galleries, stock photos, or imagery on the pages; add `ai-tool-calling` ONLY when the user explicitly asks for an AI assistant that performs actions or structured lookups via tools/function-calling — a plain conversational chatbot is `ai-chat`, not `ai-tool-calling`; add `rag-chat` ONLY when the user explicitly asks for a chat that answers from the site's OWN documents/content/knowledge base (RAG, document Q&A, 'chatbot som svarar från våra dokument') — a generic chatbot with no retrieval stays `ai-chat`, NOT `rag-chat`. Add `analytics` ONLY when the user explicitly asks for web/traffic analytics or visitor tracking (Vercel Analytics, Plausible, Google Analytics, PostHog, 'besöksstatistik') — NEVER by default and NEVER just because a site would benefit from measuring traffic; add `error-tracking` ONLY when the user explicitly asks for error/crash monitoring (Sentry, error tracking, crash reporting) — NEVER by default. For login/auth of any kind use `auth` (there is ONE auth capability; the provider — Clerk by default, Supabase on an explicit Supabase ask — is resolved downstream from the prompt, so never invent a provider-specific capability id). Add `site-search` when visitors should search the site's own content (pages, products, articles) — a cmd+k navigation palette is `command-palette`, and chat-style answers from documents are `rag-chat`. Add `map-display` when the site should show a map with markers ('hitta hit', store locations, venue) — geocoding/routing/'near me' features are NOT covered and must not force this id. Use `physics-3d` only for explicit gravity, bouncing, falling, collisions, rigid bodies or physics simulation — ordinary hovering/floating 3D stays `visual-3d`. Add `visual-3d` ONLY when the user explicitly asks for 3D, WebGL, Three.js, a canvas/mesh scene, or a GLB/GLTF model — NEVER for mood words like 'cinematic', 'immersive', 'dramatic', 'atmospheric', or 'moody' on their own; express that mood through `motionLevel`/`qualityBar` instead. `gallery-lightbox` (click-to-enlarge image gallery) and `dashboard-charts` (DATA charts/graphs or an analytics/dashboard page — NOT flow charts, org charts or process diagrams, which are ordinary page content) are explicit-ask-only. Plain content sections (FAQ, pricing table, testimonials, logo row, stats band, CTA, multi-step form UI, marquee, parallax) are ORDINARY PAGE CONTENT — describe them in `pages[].sections`, never as capability ids. Do NOT list generic structural sections like hero, about, contact, footer, or SEO, and never add a capability just because it would be nice to have.\n\n" +
