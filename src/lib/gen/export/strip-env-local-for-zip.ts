 ...
 *
 * Strip it ONLY here, at the zip/download boundary — never in the shared
 * builder and never in the verify lane. Removing it from the shared builder
 * (the approach in the superseded PR #282) regressed the verify lane: an
 * env-dependent project would pass live preview (which injects its own
 * `.env.local`) but fail/repair in the verify lane that relies on the
 * placeholder. Keeping the strip at the boundary fixes the right layer.
 *
...
export function stripGeneratedEnvLocalForZip(files: CodeFile[]): CodeFile[] {
  return files.filter((file) => normalizePath(file.path) !== ".env.local");
}
