/**
 * Dynamic-context budget splitting + priority rules.
 *
 * `splitContextIntoBudgetBlocks` parses the concatenated dynamic-context
 * string into `## Section`-delimited blocks that `buildBudgetedSystemPrompt`
 * can prune under a token budget, preserving order and respecting
 * per-section priority/required annotations.
 *
 * Extracted from `src/lib/gen/system-prompt.ts` 2026-04-21.
 */

import { estimateTokens, type PromptBudgetBlock } from "../tokens";
