...
function tryLoadFromManifest(): string | null {
  ...
  for (const rel of fragmentRels) {
    ...
    try {
      ...
      throw new Error(`[sajtmaskin] Manifest fragment missing: ${rel} → ${toPosixLog(fp)}`);
    }
...
function tryLoadMonolith(): string | null {
  for (const candidate of getMonolithCandidates()) {
    if (existsSync(candidate) && statSync(candidate).isFile()) {
      ...
      if (!raw.trim()) {
        throw new Error(`[sajtmaskin] Static system prompt file is empty: ${toPosixLog(candidate)}`);
      }
...
export function getStaticCoreFromWorkspace(): string {
  ...
  const tried = [getManifestPath(), ...getMonolithCandidates()].map(toPosixLog).join(", ");
  throw new Error(
