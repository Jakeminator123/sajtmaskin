...
| Fas | Innehåll | Mål |
|-----|----------|-----|
| **1** | Placeholder-env + `projectEnvVars` → **`.env.local` i sandbox** (genererad sajt), `npm install`, `npm run dev`; **`npm run build`** som separat verifieringsstatus | Stort lyft med rimlig omfattning |
...
- [ ] Genererad sajt får **materialiserad `.env.local`** i sandbox enligt merge-ordning i handoffen § 7.
- [ ] Preview startar **oftare** när integrationer krävs (placeholders + projekt-env).
- [ ] **Byggverifiering** (`npm run build`) rapporteras **separat** från «dev körs».

UX:

- [ ] Användaren ser **tydlig** skillnad: snabb shim ↔ runtime preview ↔ build OK / build fail (dev kan ändå köra) — **utan** att blanda in Sajtmaskins fulla interna integrationslista.
...
| Plan | Skillnad |
|------|----------|
| **PLAN-KRITIK-OPEN** | K-007 (deploy), K-009 (SSE-scope) — **inte** samma som sandbox-env; K-018 **implementeras här**. |
