...
| # | Rest | Utfall |
|---|---|---|
| P2-1 | Subprocess-/Node-helper duplicerad | **Klar, PR #647.** De fem `_resolve_node_command`-kopiorna ersattes av `resolve_node_command()` i den **nya** modulen [`backoffice/subprocess_runners.py`](../../../../../backoffice/subprocess_runners.py) — inte i `shared.py`, som bevisraden föreslog som ett av två alternativ. Bevislistan missade dessutom en andra dubblettfamilj: `resolve_command` fanns redan i `shared.py` men var kopierad ordagrant i `pipeline_health.py` och komprimerad i `eval_page.py`; båda importerar nu den delade. Modulens docstring förklarar varför de **två** probarna har olika semantik med flit — `resolve_command` faller tillbaka på original-argv så `subprocess` rapporterar felet, `resolve_node_command` returnerar `None` så sidan kan svara utan att starta en process — så nästa läsare inte slår ihop dem |
