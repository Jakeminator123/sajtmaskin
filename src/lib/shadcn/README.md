...
**API routes:** `src/app/api/shadcn/registry/{index,item,refresh}/` — proxy/cache for browser `fetch` and server refresh. `src/app/api/shadcn/community/{index,item}/` — shadcnblocks index (public metadata, bot-gate + rate limit, no cache-bypass query, `names=` capped) + item hydrate (Bearer; requires logged-in non-guest + per-user rate limit). `src/app/api/shadcn/describe/` — flag-gated Fas 1 discovery route (`POST`, default 404).
