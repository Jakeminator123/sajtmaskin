...
| Capability inference | snabb flaggning av produkt-/UI-capabilities (motion, dekorativ 3D, physics-3d, parallax, payments, auth, forms, ecommerce, charts, app shell, data UI, carousel, command search, theme toggle m.m.) | `src/lib/gen/capability-inference.ts`, `src/lib/gen/capability-dossier-bridge.ts` | prompt | `InferredCapabilities` + bridgeade dossier-capabilities (`visual-3d`, `physics-3d`, `parallax-*`, `payments`, `auth`, ...) | falska positiva på ecommerce/app shell/database; förväxling mellan dekorativ floating 3D och faktisk physics |
