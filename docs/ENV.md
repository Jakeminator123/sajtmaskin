...
| Mall-embeddings (läs-URL) | `TEMPLATE_EMBEDDINGS_BLOB_URL` | **Ingen hemlighet** — publik läs-URL till `template-embeddings.json` (~9 MiB) på Vercel Blob. Filen importeras inte längre statiskt, så den ligger utanför serverbundlen: **deployad miljö hämtar den via `fetch`**, lokal dev läser den committade filen `src/lib/templates/template-embeddings.json` via `fs`. Saknas URL:en i deployad miljö loggas ett tydligt `[template-search]`-fel och mall-sökningen faller tillbaka på nyckelordssökning (aldrig tyst tom träfflista). Uppladdningen är en ägaråtgärd: regenerera lokalt (`npm run templates:embeddings`), ladda upp till Blob, sätt nyckeln i Vercel. Källa: [`src/lib/templates/template-embeddings-storage.ts`](../src/lib/templates/template-embeddings-storage.ts). |
...
| OpenClaw edit-läge / agera-grind (opt-in) | `OC_EDIT` | **Opt-in, default av.** `OC_EDIT=1` låser upp agera-sidan: *armerad autonomi* (OpenClaw får fylla builder-prompten OCH klicka send för ett begränsat antal follow-ups, först efter användarens uttryckliga armeringshandshake; "stopp" avbryter direkt) samt Mode B bug-hunt-routen (`/api/openclaw/debug/run`, som dessutom kräver `OC_DEBUG` + `OC_DEBUG_RUN_TOKEN`). Alla redigeringar går genom builderns **ordinarie pipeline** (own-engine → verifiering → preview) — ingen direktskrivning mot preview-host/Fly och aldrig mot Sajtmaskins egen kod. Läses via `OPENCLAW.editEnabled` i `src/lib/config.ts`. |
