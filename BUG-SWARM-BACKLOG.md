...
| [ ] | Öppen bug | P1 | `SM-075` F2-gaten treatar TS2724 och TS2693 som advisory-safe: `isAdvisorySafeTypecheckOutput` släpper igenom alla olistade koder, så samma render-riskklass som TS2305/TS2614 respektive TS1361 kan promota en död preview. | `origin/master` `src/lib/gen/verify/quality-gate-checks.ts` `RENDER_RISK_TS_CODES` (rad 145–155) saknar `TS2724` och `TS2693`. | Lägg in båda koderna i setet och lås med test mot `isAdvisorySafeTypecheckOutput`. |
| [ ] | Öppen bug | P2 | `SM-076` Build-error-repair sätter `repairing` men catch anropar inte `failVersionVerification` när `files_json` är oförändrad; lease släpps i finally och versionen kan hänga tills watchdog (~950 s). `/version-status` settlar bara när bussen är `verifying`/`repairing`, inte när den redan är failed från `version.build.error`. | `origin/master` `src/lib/gen/verify/server-verify/build-error-trigger.ts` catch (rad 227–239) vs `verify-run.ts` som failar; `src/app/api/engine/chats/[chatId]/version-status/route.ts` `busStuck`. | I catch: faila versionen när snapshot är oförändrad; settla även när DB är repairing men bussen redan är terminal. |

Prioritering: `SM-072`, `SM-073`, `SM-074` och `SM-075` först; därefter `SM-076`, `SM-033`,
