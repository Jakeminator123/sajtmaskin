...
2. Lansera 8 parallella `Task`-agenter: `subagent_type: explore`, `readonly: true`, `model: <grok-4.5>`. En lane var, hårt tak på **6 tabellrader** och ingen avslutande prosa.
3. Skriv varje rå rapport till `.cursor/swarms/runs/<tidsstämpel>/r<runda>-<lane>.md`.
4. Lansera **en** destill-agent (`<grok-4.5>`, readonly) mot rundans rapporter + `FINDINGS.md`; den returnerar max 5 nya, icke-dubbletta rader. Skriv in dem med `A#<n>`-id. Gör inte hopslagningen själv — poängen är att äldre rundor aldrig behöver in i din egen kontext igen.
...
2. Lansera parallella `Task`-agenter: `readonly: true`, `model: <grok-4.5>`. Uppdrag: **motbevisa** fyndet (guard som redan finns, anropsväg som aldrig nås, fel ankare).
