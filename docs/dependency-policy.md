...
2. **Innan** on-switchen är säker återstår två saker som `gh pr merge --auto` INTE gör själv:
   - **Bot-fynd-sweep/settling:** `--auto` mergar så fort review + required checks är uppfyllda, men Codex/VADE-fynd kan landa minuter efter grönt CI (se `pr-merge.mdc` → "Författaren efter varje head-SHA"). Utan automatiserad sweep i workflow:et kan otriagerade sena fynd auto-mergas.
