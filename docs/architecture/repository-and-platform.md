...
3. **Template-källa** — mallgalleriet läser genererad katalog i `src/lib/templates/`; `scripts/template-library/sync-v0-templates.mjs` föredrar lokala `templates_v0/out`-manifest och faller annars tillbaka till v0.app-discovery. När en lokal ZIP finns i `templates_v0/downloads/` initierar builderns v0-mallflöde own-engine direkt från repo-filerna i arkivet; detta är separat från Vercel template research.
