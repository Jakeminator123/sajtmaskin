...
- **Status (forts.):** Borttagna oanvända `STREAM_RESOLVE_*`; `createOwnEnginePlanModeResponse` utan redundant `modelId`; **`generation-pipeline.ts`** kanon + `fallback.ts` re-export; delad contract-gate SSE; finalize + rollback; Vitest golden contract-gate; **`own-engine-build-session.ts`:** `buildOwnEngineGenerationStreamMeta`, **`buildPreGenerationContractGateParams`**; **`own-engine-pipeline-generation.ts`:** **`createOwnEnginePipelineAndGenerationStream`**; **`own-engine-plan-mode.ts`** (plan-mode i session-lager + `own-engine-plan-mode.test.ts`). **Kvar:** DB-transaktion / fler generation-stream-golden tests / felhantering efter lyckad version / v0-adapter-gräns om ni vill gå längre.
