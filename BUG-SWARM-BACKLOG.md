...
| [ ] | Öppen bug | P3 | import-validator destrukturerings-falskpositiv: `const { icon: PawPrint } = x` (PawPrint = lokal destrukturerad binding) matchar `ICON_PROPERTY_VALUE_RE` och kan lägga en dublett lucide-import (TS2440 — semantiskt, ej parse-fel → `runImportValidatorGuarded` reverterar ej). Sällsynt: kräver att destrukturerat lokalnamn exakt = riktigt lucide-ikonnamn. | M#cr2 | Disambiguera objekt-literal `{ icon: X }` från destrukturerings-`const { icon: X } =` innan import läggs (svårt med ren regex). Låg sannolikhet; lämnad tills repro dyker upp (`src/lib/gen/autofix/import-validator.ts`). |
## Behöver repro
