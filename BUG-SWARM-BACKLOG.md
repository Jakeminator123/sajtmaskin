...
| [ ] | Öppen bug | P2 | OC_DEBUG armerad auto-send är inte idempotent över remount: `OpenClawArmedSendCard` auto-submittar på mount via en lokal `startedRef`, så när raden remountar (t.ex. när `debugEnabled` flippar efter health-check) kan samma `submit:true`-action skicka igen och förbruka mandatsteg på dubbletter. Mandat-bindning (`createdAt`) + stop/clear-disarm finns redan men skyddar inte mot remount av samma aktuella meddelande. | BB#oc1 | Persistera ett "consumed"-märke per action/meddelande-id i `openclaw-store.ts` och skippa auto-send om redan konsumerad (`src/components/openclaw/OpenClawMessage.tsx:239-298`). Bounded av mandaträknaren idag (max 20). |
