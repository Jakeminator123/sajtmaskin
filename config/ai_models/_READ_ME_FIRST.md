...
- När du ändrar **tillåtna assist-modeller** i manifestet måste du uppdatera **`GATEWAY_ASSIST_MODELS`** och **`ANTHROPIC_ASSIST_MODELS`** i `promptAssist.ts` (eller refaktorera så listorna importeras från JSON). **`src/lib/ai-models/manifest-parity.test.ts`** varnar om de divergerar.
