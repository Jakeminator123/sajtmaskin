...
| F2/F3 quality gate på VM | **Inte env** — ligger i `config/ai_models/manifest.json` → `qualityGateTiers` | `designPreview` (F2) och `integrationsBuild` (F3) är listor av `typecheck` / `build` / `lint`. Läses i `src/lib/gen/verify/quality-gate-checks.ts`. Ändring åker med repo-commit, ingen Vercel-variabel behövs. **F2 "render is enough" (2026-07-01):** i design-läget (F2) är ett `typecheck`-**only**-fel numera *advisory* — previewn renderar → versionen promotas (verifierad) med typfelen loggade som `warning`, och repair-loopen triggas inte. Detta gäller **bara** F2; F3/integrations kör och blockar fortfarande på full `typecheck+build+lint`, och finalize-verifier-blockers stoppar även F2. Källa: `src/app/api/engine/chats/[chatId]/quality-gate/route.ts` (`designTypecheckAdvisory`). |
