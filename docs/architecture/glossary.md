...
| 2026-04-13 | Phase 1 layer consolidation (v11b): `EXTENDED_CUSTOM_INSTRUCTIONS` borttagen — visual design, layout, motion och images ägs nu av static core (`config/prompt-static/`) + brief-driven dynamic context. Inga fler duplicerade regler i custom instructions. `BUILD_INTENT_GUIDANCE` i `promptAssist.ts` markerad som assist-only med `system-prompt.ts` som canonical för codegen. Structured-prompt-heuristiker dokumenterade som separata verktyg med olika syften. |
| 2026-04-13 | Phase 1 deep consolidation (v11c): `domain-inference.ts` är ny canonical källa för domän/site-type — ersätter `inferDomainProfile` (promptAssist) och `inferSiteTypeHint`+`SITE_TYPE_RULES` (site-brief-generation). `prompt-heuristics.ts` är delad ordlistekälla för structured-prompt-heuristiker — ersätter lokala `SECTION_MARKERS`, `DESIGN_MARKERS`, `REQUIREMENT_MARKERS` (promptOrchestration) och `STRUCTURED_WEBSITE_HINTS` (server-auto-brief-policy). |
