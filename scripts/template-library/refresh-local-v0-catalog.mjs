 ...
 *
 * Usage:
 *   node scripts/template-library/refresh-local-v0-catalog.mjs
 *   node scripts/template-library/refresh-local-v0-catalog.mjs --with-embeddings
 */
...
async function main() {
  ...
  runStep("Sync local manifest into template catalog", NODE, [
    "scripts/template-library/sync-v0-templates.mjs",
    "--source=local-manifest",
  ]);

  runStep("Validate generated catalog", NODE, [
    "scripts/template-library/validate-templates.mjs",
  ]);

  if (withEmbeddings) {
    runStep("Generate template embeddings", NPM, ["run", "templates:embeddings"]);
  } else {
