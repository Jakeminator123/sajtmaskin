...
| `dashboard/` | GUI för redigering/översikt — [`domain-map.json`](dashboard/domain-map.json), kör [`dashboard/run.ps1`](dashboard/run.ps1). Detta är **konfigurationspanelen** för `config/*` och vissa docs/rules-ytor. Den visar också runtime-scaffolds, template-pipelineöversikt och prompt-dump-status, men är fortfarande inte en egen runtime-källa. Den kompletterar `scripts/scripts_dashboard.py`, som i stället är **pipeline-/artifactpanelen** för rebuild, embeddings, scaffolds, externa referenser och parity/status, samt `sajtmaskin_backoffice.py`, som är overhead-/operatörspanelen för scaffolds, eval och Autofix/Kvalitet. Delad prompt-dump-logik ligger i `scripts/dashboard_shared.py`. Delad manifest-/fault-fix-logik för Streamlit-ytorna ligger i `dashboard/shared_overhead.py`. |
