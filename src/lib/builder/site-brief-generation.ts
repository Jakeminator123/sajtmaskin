...
const BRIEF_SYSTEM_PROMPT =
  ...
  "- `requestedCapabilities` is for real implementation capabilities only. Prefer existing ids when relevant: `auth`, `payments`, `visual-3d`, `physics-3d`, `parallax-scroll`, `parallax-pointer`, `carousel`, `command-search`. Use `physics-3d` only for explicit gravity, bouncing, falling, collisions, rigid bodies or physics simulation — ordinary hovering/floating 3D stays `visual-3d`. Add `visual-3d` ONLY when the user explicitly asks for 3D, WebGL, Three.js, a canvas/mesh scene, or a GLB/GLTF model — NEVER for mood words like 'cinematic', 'immersive', 'dramatic', 'atmospheric', or 'moody' on their own; express that mood through `motionLevel`/`qualityBar` instead. You MAY also list a small set of section-capability ids — `faq-section`, `pricing-section`, `testimonials-section`, `logo-cloud`, `marquee`, `stats-counter`, `feature-grid`, `cta-section`, `gallery-lightbox`, `stepper`, `dashboard-charts` — but ONLY when the user explicitly asks for that specific section (e.g. a 'trusted by' / partner-logo row -> `logo-cloud`; a customer-quote block -> `testimonials-section`; an animated metrics band -> `stats-counter`; an image gallery -> `gallery-lightbox`; a multi-step form or wizard that needs a visible progress indicator -> `stepper`; charts, graphs, diagrams or an analytics/dashboard page -> `dashboard-charts`). Do NOT list generic structural sections like hero, about, contact, footer, or SEO, and never add a section capability just because it would be nice to have.\n\n" +
