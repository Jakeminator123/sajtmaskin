...
// ============================================================================
// Superadmin with unlimited credits - works in ALL environments (including production).
// Use for admin access and testing in production.
...
function initializeDatabase(
  ...
  // Daily rate limit tracking - prevents preview spam on v0/Vercel
  database.exec(`
    CREATE TABLE IF NOT EXISTS daily_usage (
      ...
      refines_count INTEGER DEFAULT 0,
      created_at TEXT DEFAULT (datetime('now')),
      updated_at TEXT DEFAULT (datetime('now')),
      UNIQUE(user_id, date)
    )
  ...
  // USER SETTINGS TABLE
  // ═══════════════════════════════════════════════════════════════════════════
  // Stores user preferences and API keys for external services.
  // API keys are stored encrypted (base64 + simple XOR for now, can upgrade to AES later).
  // Allows users to use their own API keys instead of platform defaults.
  // ═══════════════════════════════════════════════════════════════════════════
  database.exec(`
...
function ensureTestUserExists(database: Database.Database) {
  ...
  // Also ensure superadmin exists (works in ALL environments including production)
  ensureSuperadminExists(database);
}

// Ensure superadmin exists - works in ALL environments
// Superadmin has unlimited diamonds and is_superadmin flag set
function ensureSuperadminExists(database: Database.Database) {
  if (!SUPERADMIN_EMAIL || !SUPERADMIN_PASSWORD) {
    debugLog(
      "[Database] No superadmin configured (set SUPERADMIN_EMAIL and SUPERADMIN_PASSWORD in env)"
    );
    return;
  }

  const existingUser = database
    .prepare("SELECT id, is_superadmin FROM users WHERE email = ?")
    .get(SUPERADMIN_EMAIL) as { id: string; is_superadmin: number } | undefined;
  ...
  if (!existingUser) {
    ...
    debugLog("[Database] Created superadmin:", { email: SUPERADMIN_EMAIL });
    ...
    debugLog("[Database] Upgraded user to superadmin:", { email: SUPERADMIN_EMAIL });
  }
}
...
export interface DailyUsage {
  ...
  refines_count: number;
  created_at: string;
  updated_at: string;
}

// Rate limits per day (superadmin/test users bypass these)
export const DAILY_RATE_LIMITS = {
  generations: 100,  // Max generations per day per user
  refines: 500,      // Max refinements per day per user
} as const;
...
// Get or create daily usage for user
export function getOrCreateDailyUsage(userId: string): DailyUsage {
  const db = getDb();
  const today = getTodayDateString();
  
  const existing = getDailyUsage(userId);
  if (existing) return existing;
  
  // Create new daily record
  const stmt = db.prepare(`
    INSERT INTO daily_usage (user_id, date, generations_count, refines_count)
    VALUES (?, ?, 0, 0)
  `);
...
export function canUserGenerate(userId: string): { allowed: boolean; remaining: number; limit: number } {
  // Superadmins bypass rate limits
  const user = getUserById(userId);
  if (user?.is_superadmin || user?.email === TEST_USER_EMAIL) {
    return { allowed: true, remaining: DAILY_RATE_LIMITS.generations, limit: DAILY_RATE_LIMITS.generations };
  }
  
  const usage = getOrCreateDailyUsage(userId);
  const remaining = DAILY_RATE_LIMITS.generations - usage.generations_count;
  
  return {
    allowed: remaining > 0,
    remaining: Math.max(0, remaining),
    limit: DAILY_RATE_LIMITS.generations,
  };
}

// Check if user can refine (rate limit check)
export function canUserRefine(userId: string): { allowed: boolean; remaining: number; limit: number } {
  // Superadmins bypass rate limits
  const user = getUserById(userId);
  if (user?.is_superadmin || user?.email === TEST_USER_EMAIL) {
    return { allowed: true, remaining: DAILY_RATE_LIMITS.refines, limit: DAILY_RATE_LIMITS.refines };
  }
  
  const usage = getOrCreateDailyUsage(userId);
  const remaining = DAILY_RATE_LIMITS.refines - usage.refines_count;
  
  return {
    allowed: remaining > 0,
    remaining: Math.max(0, remaining),
    limit: DAILY_RATE_LIMITS.refines,
  };
}

// Increment daily generation count
export function incrementDailyGenerations(userId: string): DailyUsage {
  const db = getDb();
...
// Increment daily refine count
export function incrementDailyRefines(userId: string): DailyUsage {
  const db = getDb();
  getOrCreateDailyUsage(userId);
  const today = getTodayDateString();
...
// Check if user is the test user or superadmin with unlimited credits
export function isTestUser(user: User | null): boolean {
  if (!user) return false;
  return user.email === TEST_USER_EMAIL || user.is_superadmin === true;
}
...
interface UserSettingsRow {
  ...
  enable_thinking_display: number;
  created_at: string;
  updated_at: string;
}
 ...
 * For production, consider using proper encryption with a secret key
 */
...
export function getUserSettings(userId: string): UserSettings | null {
  ...
  return {
    ...
    enable_thinking_display: row.enable_thinking_display === 1,
    created_at: row.created_at,
    updated_at: row.updated_at,
  };
}
...
/**
 * Get or create user settings (returns defaults if not exists)
 */
...
export function updateUserSettings(
  ...
  if (settings.preferred_quality !== undefined) {
    updates.push("preferred_quality = ?");
    values.push(settings.preferred_quality);
  }

  if (settings.enable_streaming !== undefined) {
    updates.push("enable_streaming = ?");
    values.push(settings.enable_streaming ? 1 : 0);
  }

  if (settings.enable_thinking_display !== undefined) {
    updates.push("enable_thinking_display = ?");
    values.push(settings.enable_thinking_display ? 1 : 0);
  }
 ...
 */
export function hasCustomApiKeys(userId: string): boolean {
