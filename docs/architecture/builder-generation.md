...
- Efter stream: `finalizeAndSaveVersion`, autofix-pipeline (loopas `DETERMINISTIC_AUTOFIX_MAX_PASSES` gånger, manifest-styrt med 15 deterministiska fixers inkl. lucide-link/image-fixers), syntaxvalidering (`validateAndFix`, eskalerar till LLM-fixer vid behov men stoppar tidigt vid fixer-noop eller utebliven förbättring), ev. verifier + polish-pass, och capad/bounded-parallel bildmaterialisering i deep path, kvalitetsgrind — se `generation-loop-and-error-memory.md` i arkivet och `llm-pipeline-flow.html` för visuellt flöde.
