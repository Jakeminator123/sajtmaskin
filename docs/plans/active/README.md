...
Levererat/beslut (inspector "Inspektera preview" rendering-arkitektur, arkiverad 2026-07-13): **Option B (instrumenterad preview / inspect-bridge) vald och levererad flagg-gated (PR #164, default av); Option A (render-inspector-worker på Render.com) avvisad, workern raderad.** Kvar (litet pass, ej blockerande): live-verifiera bryggan + pensionera gamla `/api/inspector-{capture,element-map}` + ev. CSP `frame-src *.fly.dev` → [`../avklarat/2026-06-19-inspector-rendering-arkitektur.md`](../avklarat/2026-06-19-inspector-rendering-arkitektur.md).
...
Levererat (dossier legacy-import 2026-07-08, arkiverad 2026-07-13): normalizer + backoffice-flik (#419) + **alla 12 utkast promoterade** — sista 4 (sanity-cms/paddle-billing/supabase-auth/rag-chat) via #475; F2 DB-mock löst av deklarativt mock-läge (#468) + CI-invariant per hard-capability (#499/#502). Kvar (svans, ej blockerande): preview-test → bumpa `lastVerified` på dossiers som står på legacy-datum → [`../avklarat/2026-07-08-dossier-legacy-import.md`](../avklarat/2026-07-08-dossier-legacy-import.md).

**Levererat (2026-07-12, PR #498–#504):** Dossier-grupper (10 st, presentations-lager) + fallback-kontrakt — terminologi/docs, gruppmapp 9→10, CI-invariant (per-dossier: **varje** hard-dossier ⇒ `mock ≠ none` eller capability-undantag), backoffice-kategorivy med lägg-till/radera, catalog/API-röktest. **Etapp 7-kärnan levererad:** P1 ReleaseGate TOCTOU fixad (#504), F3-approve installerar vilande integrationskod via dossier-injektion (#503, `BB#f3det1/2`); capability-provenance ägarbeslutad-deferred. Designnotat 7.2 + öppet 7.3-residualscope (beteendetester per hard-dossier + aktiverings-E2E) i planfilen → [`../avklarat/2026-07-12-dossier-grupper-och-fallback-kontrakt.md`](../avklarat/2026-07-12-dossier-grupper-och-fallback-kontrakt.md).

Levererat (OpenClaw edit-agent 2026-07-01, arkiverad 2026-07-13): 2 av 3 spår — mallar→Blob #336 mergad, follow-up imported-repo-fix i koden. Spår A **parkerat** (PR #346 stängd utan merge 2026-07-09, "kanske senare"; branch `feat/openclaw-edit-agent` raderad, ingen kod på master) → [`../avklarat/2026-07-01-openclaw-edit-agent-och-followup-fix.md`](../avklarat/2026-07-01-openclaw-edit-agent-och-followup-fix.md).

**Aktiv (huvuddrivlinje 2026-07-13)** — Stabilisering: **PR 1–3 levererade samma dag** (#517 F3-env-gate M#f3env1 · #518 en verify-ägare per F2-version M#vlane1–2/BB#299 · #519 domänrester #486 a–c/BB#deploy4 — arkivrader i [`backlog-arkiv-2026-07-02.md`](../avklarat/bug-swarm/backlog-arkiv-2026-07-02.md)). **Kvar i planen:** PR 4 review-freshness, PR 5 verification-invalidation, § 6-svansar från det raderade lokala kördokumentet `builder-stabilisering` (F3-kravyta-slut, env-scope-fallback, `configured`-init, single-canary) → plan [`2026-07-13-stabilisering-verify-f3-doman-plan.md`](2026-07-13-stabilisering-verify-f3-doman-plan.md).

**Aktiv (ny 2026-07-13, från /logg-session chat 747636c8)** — fyra scope:ade planer efter ägar-observationer om env-plotter + UI-brus + en prod-500-storm (väntar på prioritering/beslut, ingen implementation påbörjad):

- Användarsajternas env → **en** kanonisk yta + möjlighet att sätta riktiga värden i F2 (rör medveten `env-flow-f2-mute`-policy → kräver beslut) → [`2026-07-13-anvandarsajt-env-konsolidering.md`](2026-07-13-anvandarsajt-env-konsolidering.md).
- Builder-UI declutter: ReleaseGate-/Designpreview-bannrar → logg, F2 env-box bort (behåll `canDeploy`-grind + autofix-trigger) → [`2026-07-13-builder-status-ui-declutter.md`](2026-07-13-builder-status-ui-declutter.md).
- Runtime-robusthet: DB-pool-500-storm (mjuk degradering + klient-backoff + pool-mätning), CSP/font-brus, scaffold-lint-bugg `use-reduced-motion` som fäller ReleaseGate → [`2026-07-13-builder-runtime-robusthet.md`](2026-07-13-builder-runtime-robusthet.md).
- Dossier/UI-ownership-kontrakt (M#dchat1): en tidigare LLM-byggd chatt-yta får inte konkurrera med en senare `openai-chat`-dossier — adapt-eller-ersätt, aldrig två ytor/routes; inkl. regressionstest-spec → [`2026-07-13-dossier-ui-ownership-kontrakt.md`](2026-07-13-dossier-ui-ownership-kontrakt.md).

**Aktiv** — Backoffice-stringensplan 2026-07-08 (underlag klart: sidor granskade + 3 triviala sanningsfixar; resten väntar på prioritering — domain-map-täckning, dubbla manifest-editorer, scaffold-navigation, testluckor). **OBS: nulägesbilden är delvis stale** — panelen växte 34→37 sidor och dossier-ytan byggdes om (#500) senaste dagarna; friska upp innan implementation via refresh-brief:et → plan [`2026-07-08-backoffice-stringens-plan.md`](2026-07-08-backoffice-stringens-plan.md), refresh-handoff [`2026-07-13-backoffice-stringens-refresh-handoff.md`](2026-07-13-backoffice-stringens-refresh-handoff.md).

Levererat (2026-07-09, PR #456): Publicera-stringensplan — publicera/deploy + F3-integrationer stringentare (ReleaseGate-lås, domänlås, deploy-repair-loop, inspectorUrl/felstate, RAG-härdning, terminologi) + kompletteringspass A#12/A#5/A#3. Kvar: bug-backlog `BB#deploy5` (i [`BUG-SWARM-BACKLOG.md`](../../../BUG-SWARM-BACKLOG.md); `BB#deploy2` stängdes i #469, `BB#deploy3` i #456, `BB#deploy4` i #519) → [`../avklarat/2026-07-08-publicera-stringens-plan.md`](../avklarat/2026-07-08-publicera-stringens-plan.md).
