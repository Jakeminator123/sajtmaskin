...
**Operativ kö (läs först):** [`MASTER-ALLT-KVAR.md`](./MASTER-ALLT-KVAR.md) — allt kvar; kanonisk K-tabell **här**; **K-018**-detalj även [`queue/PLAN-PREVIEW-SANDBOX.md`](./queue/PLAN-PREVIEW-SANDBOX.md).
...
|----|-------------------------|-------------|-----|------|--------|
| K-007 | `78pct-r`, `64pct-s`, progress | W2-rester: deploy auto-fix / hårdare valideringsfas före deploy (valfritt produktbeslut) | kod / produkt | hög | [ ] — **beslut 2026-03-26:** policy **oförändrad** — se [`../avklarat/2026-03-handoff-doc-bundle/BESLUT-INNAN-VI-GAR-VIDARE.md`](../avklarat/2026-03-handoff-doc-bundle/BESLUT-INNAN-VI-GAR-VIDARE.md) §1. **delmoment 2026-03-26:** `GET` readiness spärrar om **strikta JSON-filer** i versionen (`package.json`, `components.json`, `jsconfig.json`) är ogiltiga; se `version-file-integrity.ts` |
| K-008 | `78pct-r`, `34pct-n` §5 (historisk) | Landningspolish / W1-rester (historisk rad) | mixed | medel | [x] 2026-03-25 — produkt: **ingen utökning** av landningsmaterial nu; fokus **användarsidor + iframe** (**K-018**) |
| K-009 | `78pct-r`, `64pct-s` | Own-engine **utanför** avslutad W3-track (SSE, produkt) — **inte** marknads-FAQ på Sajtmaskin (sekundärt) | kod | hög | [ ] — **beslut 2026-03-26:** scope/tolkning — se [`../avklarat/2026-03-handoff-doc-bundle/BESLUT-INNAN-VI-GAR-VIDARE.md`](../avklarat/2026-03-handoff-doc-bundle/BESLUT-INNAN-VI-GAR-VIDARE.md) §2 (rad **[ ]** tills leverans eller **[N/A]** efter medveten review) |
| K-018 | ägarbeslut (FAQ B1/C1) | **Användargenererade** sidor: preview ska hålla **npm run dev**-klass som React; **samma fidelity i `iframe`** | kod / produkt | hög | [ ] — **beslut 2026-03-26:** mätning + prioriteringsordning — se [`../avklarat/2026-03-handoff-doc-bundle/BESLUT-INNAN-VI-GAR-VIDARE.md`](../avklarat/2026-03-handoff-doc-bundle/BESLUT-INNAN-VI-GAR-VIDARE.md) §3. **delmoment 2026-03-26:** `.env.local` merge; valfritt **`npm run build`** i sandbox via `SAJTMASKIN_SANDBOX_PREVIEW_MODE=dev_then_build` (default i kod nu **`dev_only`** = tier 2). SSE `prodBuildVerified` när build körs. **2026-03-26 (senare):** `warnLog` + toast vid sandbox-fallback till shim; parse/tomt projekt → `build-error`. **Preview fidelity tier 1–3:** `docs/architecture/preview-fidelity-tiers.md`. **2026-03-26:** Fas 2 **in-memory** `sandbox-session-store` + `touchSandboxSession` vid lyckad `startSandboxPreview` (`chatId`). Kvar: **sann** VM-återanvändning (SDK/plattform), shim↔runtime **produkt**-prioritet, Fas 3 adapters (MASTER § 2). |
| K-019 | `.j_to_agent/fidelity.txt` + [`MASTER-ALLT-KVAR.md`](./MASTER-ALLT-KVAR.md) § 0 | **Builder standard-UX + promptkedja:** Agentlogg/klarifieringar/råtext **sekundära** i standardläge (kvar för debug); **kanonisk** förberedd orchestration-kontext **persisteras** över follow-up/närliggande stream-steg så kontext inte tappas eller dupliceras | kod / produkt | hög | [ ] — **beslut 2026-03-26:** DB, merge, storlek — se [`../avklarat/2026-03-handoff-doc-bundle/BESLUT-INNAN-VI-GAR-VIDARE.md`](../avklarat/2026-03-handoff-doc-bundle/BESLUT-INNAN-VI-GAR-VIDARE.md) §4. **delmoment 2026-03-26:** Agentlogg **hopfälld** som standard (`AgentLogCard`). **2026-03-26 (senare):** `engine_chats.orchestration_snapshot` + sanering (`orchestration-snapshot.ts`); sparas efter lyckad `finalizeAndSaveVersion`; follow-up (`POST .../stream`) **prependar** kontinuitet. **Kvar:** finare merge-policy / ev. UI — se [`queue/PLAN-K019-PROMPT-SNAPSHOT.md`](./queue/PLAN-K019-PROMPT-SNAPSHOT.md). Kräver DB-migration `add-engine-chat-orchestration-snapshot.sql`. |
| K-014 | `18pct-k` (arkiverad sammanfattning) | Footer / juridik / cookies — produktcopy och eventuella sidor | copy | medel | [x] 2026-03-26 — produkt: sidor (t.ex. cookies, om oss, juridik) **OK oförändrat tills vidare** |
