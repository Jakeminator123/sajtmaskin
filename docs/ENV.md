...
| Preview-förvärmning (host-väckning + överlappad install) | `SAJTMASKIN_PREVIEW_PREWARM=true` | **Opt-in, default av.** En ny chats första riktiga codegen-körning kan väcka preview-hosten och starta baseline-installationen medan LLM-strömmen pågår. Plan-mode, kontraktsklargörande och vanliga follow-ups förvärms aldrig. Finalize skippar install endast när `package.json`/lockfiler matchar baseline-fingerprintet; dossierberoenden kan ändra fingerprintet och ge en andra install. Kräver `SAJTMASKIN_PREVIEW_HOST_BASE_URL`. Rekommenderad utrullning är **Development/Preview först**, i linje med `config/env-policy.json`; mät fingerprint-träffar och installkö innan Production. Källa: `src/lib/gen/preview/preview-prewarm.ts` (`FEATURES.previewPrewarm`). |
