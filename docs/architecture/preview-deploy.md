...
4. Vid lyckad readiness: **`engine_versions.sandbox_url`** sätts; klienten visar **Fidelity 2** — `fidelityTier: 2` i `SandboxPreviewResult` (riktig Next dev-server i VM). **Sandbox är primär previewväg.** Shim (`/api/preview-render`) är bara **kompatibilitetsvy** under migration/fallback, inte målpreview.
...
| Sandbox-first | `done` kan sakna `demoUrl` medan `sandboxPending` är true; UI ska visa live-preview-status i stället för att låtsas att shim är huvudpreview | `generation-stream.ts`, `stream-handlers.ts`, `PreviewPanel.tsx` |
...
1. Efter `done` i SSE är **sandbox** den tänkta previewytan. `demoUrl` kan därför vara `null` medan `sandboxPending` är true. `shimPreviewUrl` är bara en **kompatibilitets-URL** under migrationen, inte den primära vägen.
...
**Feature flag:** `SAJTMASKIN_SANDBOX_ONLY_PREVIEW=1` stänger av kompatibilitets-shim i preflight-kontraktet och gör sandbox till enda previewyta.
