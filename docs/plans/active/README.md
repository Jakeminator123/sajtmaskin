...
| Kostnadsfri-kampanjflödet | **Alla tre besluten fattade; inget i `master`.** Sidantal (kampanjstandard 3 via strukturerad `pageCountHint`, uttryckliga val på 1–2 bevarade) levererat till `preview` via #1370 (`53daaa6eb`). Bolagsdatans PII-gräns beslutad och ingesten byggd — i PR. Pre-generering avgjord: ingen spekulativ init vid lösenordsverifiering. Kvar: wizardens förifyllning och taxonomins lib-ägare (staplad PR), samt `preview`→`master`. | [`2026-09-14-kostnadsfri-kampanjflode/00-master-plan.md`](2026-09-14-kostnadsfri-kampanjflode/00-master-plan.md) |
