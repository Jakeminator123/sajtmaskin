...
- [~] **Omr 2 `S3` statusresolver-invariant:** aktiverad som hård import-invariant i **6-3 punkt 2** (`status-resolver-single-writer.stability.test.ts`) + legacy-resolvern `resolveEngineVersionDisplayStatus` borttagen. Levererad i draft-PR #163 (ej mergad).
...
- [~] **Område 6 (§6 steg 4) pågår:** `6-1` (#159, `b9eb43297`) + `6-2` (#160, `0c27010ed`) **mergade** — event-bus-status-flip klar för shell + preview-empty-state (mapper `version-status-display.ts`, false-green-vakt: degraderad ≠ grön) **och** `VersionHistory`-badgen (server-enrichat `busStatus` på `/versions` + label-lager `version-history-status-labels.ts`); legacy `resolveEngineVersionDisplayStatus` ej längre konsumerad i app-kod. `6-3 punkt 1` (#162, `879ac0c6b`) mergad (false-green-fix); `6-3 punkt 2` (denna draft-PR #163) aktiverar S3 som hård import-invariant (`status-resolver-single-writer.stability.test.ts`) + tar bort den döda legacy-resolvern + typen — Område 6 därmed levererad (draft pending merge). Backlog: `readAll`-per-poll-perf på `/versions` (VADE-fynd, eget pass — se `_loggbok.md`). Stänger N#6/G#32.
