  ...
  return withRateLimit(req, "v0:deployments-list", async () => {
    try {
      ...
      if (
        ...
        if (configured === false) {
          await clearProjectCustomDomainVerification(appProjectId, appProject.custom_domain);
      ...
      if (
        ...
        if (configured === true) {
          ...
          await clearProjectBrandedDomainVerification(appProjectId, appProject.branded_domain);
      ...
      if (latestRefreshCandidate?.vercelDeploymentId) {
        try {
          ...
          refreshedById.set(latestRefreshCandidate.id, {
            ...
            providerUrl: vercel.url ?? latestRefreshCandidate.providerUrl ?? null,
            url: refreshedLiveUrl ?? resolveLegacyProviderUrl(latestRefreshCandidate.url),
            inspectorUrl: vercel.inspectorUrl ?? latestRefreshCandidate.inspectorUrl ?? null,
            vercelProjectId:
              vercel.vercelProjectId ?? latestRefreshCandidate.vercelProjectId ?? null,
          });
        } catch (err) {
