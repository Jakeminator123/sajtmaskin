  ...
  try {
    ...
    if (dbFiles && dbFiles.length > 0) {
      ...
      try {
        ...
        console.warn("[project-files] Failed to cache SQLite files in Redis:", cacheError);
      }
    ...
    if (
      ...
      if (filesFromLegacy.length > 0) {
        ...
        try {
          ...
          console.warn("[project-files] Failed to persist legacy files to SQLite:", persistError);
        }
        ...
        try {
          ...
          console.warn("[project-files] Failed to cache legacy files in Redis:", cacheError);
        }
