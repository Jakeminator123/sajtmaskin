...
| v0/engine route consolidation (P29) | Sedan 2026-04-20: 18 av 28 v0-chat-routes (rena `export { GET } from "@/app/api/engine/..."`-re-exports utan egen test-coverage) borttagna ur `src/app/api/v0/chats/**`. Audit `03-konsolidering-pipeline.md` §3.4 / Tier A #14. Kvar: 10 routes vars test-fil innehåller assertions engine-sidan saknar — kräver test-migrering till engine-side innan deletion (Fas 1B i `docs/plans/active/P29-v0-engine-consolidation.md`). 7 unika v0-routes (Class C: `init-registry`, `integrations/vercel/projects`, `projects/instructions`, `projects/[id]/env-vars`, `deployments/*`) är legitim legacy med faktiska klient-callsites — väntar på Fas 2-beslut (rename till `/api/legacy/v0/*` eller behåll). Klient-prefixen för aktiv chat-trafik är `ENGINE_CHATS_API_PREFIX` i `src/lib/api/engine-chats-path.ts`. | kanonisk |
