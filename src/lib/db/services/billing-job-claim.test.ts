    ...
    update: () => ({
      set: () => ({
        where: (clause: unknown) => {
          claimDb.updateWhereSpy(clause);
          return {
            returning: () => {
              const claimed = claimDb.applyAtomicClaim(claimDb.jobStore.current, clause);
              if (claimed) claimDb.jobStore.current = claimed;
...
describe("claimRunnableBillingJob stale-list race", () => {
  beforeEach(() => {
    claimDb.updateWhereSpy.mockClear();
    claimDb.jobStore.current = null;
  });
  ...
  it("låter inte worker B reclaima via stale lista innan 15-minutersbackoff", async () => {
    ...
    const listSnapshot = {
      status: claimDb.jobStore.current.status,
      runAfter: claimDb.jobStore.current.run_after,
      leaseExpiresAt: claimDb.jobStore.current.lease_expires_at,
      now: listedAt,
    };
    const workerAList = [{ id: claimDb.jobStore.current.id, ...listSnapshot }];
    const workerBList = [{ id: claimDb.jobStore.current.id, ...listSnapshot }];
    ...
    expect(claimDb.extractRunAfterGate(claimDb.updateWhereSpy.mock.calls.at(-1)?.[0])?.getTime()).toBe(
      listedAt.getTime(),
    );
    expect(claimedByA?.status).toBe("running");
    expect(claimDb.jobStore.current?.attempts).toBe(1);
    ...
    expect(claimDb.jobStore.current.status).toBe("failed");
    expect(claimDb.jobStore.current.run_after.getTime()).toBe(backoffUntil.getTime());
    ...
    expect(claimDb.jobStore.current.attempts).toBe(2);
  });
});
