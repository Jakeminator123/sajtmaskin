...
`@clerk/nextjs` tillhör den *genererade* användarsajten (Byggblocket `clerk-auth`). Sajtmaskin själv har inte paketet installerat. Därför aliasar `tsconfig.json` och `vitest.config.ts` importen hit, så dossier-komponenttester kan resolva utan riktig SDK.
