...
| Prod-migrationer (`scripts/db/migrate-prod.mjs`)                      | `prod-migrations-apply`                                                                            | push till `master` eller `preview` + manuell dispatch (aldrig PR — prod-secret injiceras inte på `pull_request`) | Gate:ad bakom `quality` + `schema-drift`   | Preview delar prod-Postgres; migrationer körs inte av Vercel-deployen; idempotent + bokför `schema_migrations`-ledgern                                                                                                                          |
| Additiv-bara-grind (`scripts/db/check-additive-migrations.mjs`)       | steg i `prod-migrations-apply`                                                                     | endast push till `preview` (före applyn)                                                                        | **Ja** — hård gate på preview              | Preview delar prod-DB med en produktion som kör gammal kod: brytande DDL från staging måste köras medvetet via promote, inte automatiskt. Granskar bara **pending** migrationer                                                                |
