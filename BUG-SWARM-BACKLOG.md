...
| [ ] | Öppen bug | P2 | `SM-013` Misslyckad template-init lämnar `?templateId=...` utan `chatId`; tomläget fortsätter därför visa ”Läser in templaten” tills reload. | `useBuilderEffects.ts`, `PreviewPanelEmptyState.tsx` och `POST /api/template`. | Idempotent POST på `(projectId eller owner) + templateId`, explicit felläge i tomläget och «Försök igen» som replayar samma nyckel. Arkivera vid merge. |
...
| [ ] | Öppen bug | P2 | `SM-085` Sen product-postcheck/live review av en äldre version läser chattens SENASTE user-prompt som `userRequest` i stället för den prompt som skapade versionen, så granskningen kan döma v1 mot uppföljningens önskemål. | `src/lib/gen/verify/live-review.ts` (`resolveUserRequestForVersion`) och `src/app/api/engine/chats/[chatId]/product-postcheck/route.ts`. Observerad 2026-09-01 (chat `5efde3c4`): en sen resume bedömde v1 mot uppföljningens prissektionskritik. | Pinna `userRequest` till den user-turn som skapade `versionId` (via `version.message_id`, därefter `created_at`). Latest-user är synlig fallback (log), inte tyst gissning. Rotorsaken är en annan än `SM-077`:s uteblivna omkörning. |
| [ ] | Öppen bug | P2 | `SM-086` Desktop-skärmbilden för live review kan bli `null` utan något fynd, och granskningen körs vidare som om båda viewportarna fanns. | `src/lib/gen/verify/product-postcheck.ts` (`capturePostcheckJpeg`) och `src/lib/gen/verify/live-review.ts` (`screenshotViewportCoverage`). Observerad 2026-09-01 (chat `5efde3c4`): v1 fick `has_desktop: false` medan v2/v3 fick båda. | Ett desktop-retry + varning när `desktopUrl` saknas. Live review körs advisory på ofullständig set men labelar bilder och säger `mobile_only` — inte «båda viewportar». |
