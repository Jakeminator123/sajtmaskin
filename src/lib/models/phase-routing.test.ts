...
describe("resolvePhaseModel", () => {
  ...
  it("downgrades planner and verifier for pro tier", () => {
    ...
    expect(planner.modelId).toBe("gpt-4.1-mini");
    expect(verifier.modelId).toBe("gpt-4.1");
    expect(generator.modelId).toBe("gpt-5.3-codex");
    expect(fixer.modelId).toBe("gpt-5.3-codex");
  });

  it("downgrades planner and verifier for max tier", () => {
  ...
  it("downgrades planner and verifier for codex tier", () => {
...
describe("getPhaseRoutingSummary", () => {
  ...
  it("returns distinct models for pro tier planner vs generator", () => {
    const summary = getPhaseRoutingSummary("pro");
    expect(summary.planner).toBe("gpt-4.1-mini");
