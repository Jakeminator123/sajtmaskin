...
| Capability single-source | G#25, G#26, N#2 | **4** Prompter + **1** Kontrakt + **5** | Wave 2 | Konsolidering vald, men canonical-källan måste spänna **init + follow-up + dossier-bridge + `orchestrate.ts`** (signal-ägarmatris) — **inte** bara `FollowUpContract.capabilities` (finns bara på follow-up-pathen → lämnar init-drift olöst). **Delvis 2026-06-21:** snapshot-**läs**-sidan nu konsekvent — `resolveSelectedDossiersFromSnapshot` (rank 3, #181) + `buildFollowUpContract`-golvet (rank 4, #183) läser snapshotens top-level mergade `requestedCapabilities` i st.f. `briefSummary`-delmängden. Init-drift-delen kvar. |
