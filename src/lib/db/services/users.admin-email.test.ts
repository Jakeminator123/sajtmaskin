...
describe("isAdminEmail", () => {
  it("matches a mixed-case, padded SUPERADMIN_EMAIL", () => {
    setPrivilegedEnv({ superadminEmail: "  Privileged@Example.TEST " });

    expect(isAdminEmail("privileged@example.test")).toBe(true);
    expect(isAdminEmail("  PRIVILEGED@example.test  ")).toBe(true);
  });

  it("matches a mixed-case TEST_USER_EMAIL", () => {
    setPrivilegedEnv({ testUserEmail: "Tester@Example.test" });

    expect(isAdminEmail("tester@example.test")).toBe(true);
  });
