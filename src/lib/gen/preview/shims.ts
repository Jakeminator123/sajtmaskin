...
export function buildPreviewPrelude(modules: PreparedModule[], routePath: string): string {
  const lines: string[] = [
    ...
    "const dynamic = (importFn, opts) => {",
    "  const Lazy = React.lazy(function () {",
    "    return Promise.resolve(importFn()).then(function (m) {",
    "      const Def = m && m.default;",
    "      if (typeof Def === 'function') return { default: Def };",
    "      return { default: function PreviewDynamicEmpty() { return null; } };",
    "    });",
    "  });",
    "  let loadingFallback = null;",
    "  if (opts && typeof opts.loading === 'function') {",
    "    try { loadingFallback = React.createElement(opts.loading, null); } catch (e) { loadingFallback = null; }",
    "  }",
    "  if (!loadingFallback) {",
    "    loadingFallback = React.createElement('div', { style: { padding: '1rem', color: '#64748b', fontSize: '13px' } }, 'Laddar komponent…');",
    "  }",
    "  return function PreviewDynamicWrapper(props) {",
    "    return React.createElement(React.Suspense, { fallback: loadingFallback }, React.createElement(Lazy, props));",
    "  };",
    "};",
    ...
    "}",
    "var cn = __previewCn;",
    "var clsx = __previewCn;",
    "var twMerge = __previewCn;",
