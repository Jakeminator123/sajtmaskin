...
- **STORDSTAD-spår + separat worktree:** om du har en **andra checkout** (t.ex. `…\sajtmaskin-stordstad-master` på branch `stordstad-master-resume`), öppna **endast** den mappen eller **`stordstad-only.code-workspace`** i den roten — **inte** huvudmappen `…\sajtmaskin` i samma multi-root-fönster, om du vill att statusraden ska visa **en** branch. Annat arbete (t.ex. `llm-pipeline-upgrade`) = **eget** Cursor-fönster. Se `docs/plans/active/STORDSTAD-repo-kod-databas.md` § Handoff → *Cursor: var du arbetar*.
