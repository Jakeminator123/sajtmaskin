...
Briefen är den **kanoniska semantiska expansionen** för init. Brief-objektet skickas via `meta.brief` och konsumeras av `buildDynamicContext()`. Brief-deriverad prose sammanfogas inte längre med `customInstructions` — `customInstructions` bär enbart användarens egna instruktioner. Server Auto-Brief (`shouldRunServerAutoBrief`) körs som fallback för underspecificerade init-prompts (inklusive korta vaga website-prompts); skip kvarstår för audit, technical, follow-up och redan tydligt strukturerade prompts. **Follow-ups återkör inte Deep Brief-LLM:en** — när `meta.brief` saknas hydreras en minimal snapshot-brief (requestedCapabilities, domainProfile-slug, visualDirection.styleKeywords, toneAndVoice, projectTitle, brandName) från `orchestration_snapshot.briefSummary` via `buildFollowUpBriefFromSnapshot()`. Följ-upen förlitar sig alltså på persisted scaffold, snapshot-brief och tidigare filer.
