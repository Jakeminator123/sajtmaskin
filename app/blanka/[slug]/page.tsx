...
export default async function BlankaPostPage({ params }: PageProps) {
  const { slug } = await params;
  const post = await getPostBySlug('blanka', slug);
      ...
      <main className="flex-1">
        <article className="py-12 sm:py-16">
          <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            <Breadcrumbs
              items={[
                { label: 'Blanka blogg', href: '/blanka' },
                { label: post.title },
              ]}
            />
            ...
            <div className="mb-8">
              <Button asChild variant="ghost" size="sm" className="mb-6">
                <Link href="/blanka">
                  ...
                  Tillbaka till Blanka blogg
                </Link>
              </Button>
            </div>

            {/* Post Header */}
            <header className="mb-12 rounded-3xl bg-gradient-to-br from-blanka-teal-bg to-blanka-teal-light/30 p-8 sm:p-12">
              <div className="mb-6 flex flex-wrap items-center gap-3">
                <Badge className="bg-gradient-to-r from-blanka-teal to-blanka-teal-dark text-white border-0 shadow-md px-3 py-1 font-bold">
