...
| Scaffold template guidance | `SAJTMASKIN_RUNTIME_TEMPLATE_GUIDANCE=true` | Scaffold-ankrad template-library guidance för init-generering. Hämtar kompakt `runtimeGuidance` från scaffoldens `referenceTemplates` i `template-library.generated.json` och injicerar i `## Scaffold Research Priorities`. Default av. Kan även styras via backoffice (Research & Dossiers). Runtime läser **inte** råa dossiers. |
| Deferred extra init routes | `SAJTMASKIN_DEFER_EXTRA_ROUTES_ON_INIT=true` | Opt-in för att låta init-genereringar planera flera routes men bara fullt realisera primärrouten direkt. Extrasidor blir då giltiga shells med tydlig `Skapa sida`-yta i stället för att kräva full implementation på första passet. Default av. |
