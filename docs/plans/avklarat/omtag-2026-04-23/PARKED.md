...
| **UI event-bus-flip (Kvarvarande #11)** | `STATUS-2026-04-23-omtag-complete.md` punkt C | Flippa `VersionHistory.tsx` + `BuilderShellContent.tsx` till `selectVersionStatus(events)`-projektionen. 2026-04-23 levererades quick-fix i `runner.ts` + `server-verify.ts` som löser den akuta "Fel innan server-verify klar"-inversionen utan arkitektur-flip. Full flip kvarstår. | Egen session, 1–2 dagar |
