  ...
  return (
    ...
    >
      <div className="border-b border-violet-900/40 px-3 py-2">
        {error ? (
          // Felet hör hemma här, inte i stället för listan: `items` startar med
          // de åtta fröna och är därför aldrig tom, och en frö-post går att
          // sätta in ändå. Ersatte vi listan skulle ett hämtningsfel dölja
          // åtta fungerande kort — och ett fel på en OMLADDNING efter lyckad
          // hämtning syntes inte alls.
          <div className="flex flex-wrap items-center gap-1.5 text-[11px] text-rose-200/90">
            <AlertCircle className="h-3.5 w-3.5 shrink-0 text-rose-400" />
            <span>{error}</span>
            <button
              type="button"
              onClick={() => setReloadToken((n) => n + 1)}
              className="rounded border border-violet-800/60 px-1.5 py-0.5 text-[10px] text-violet-200 transition hover:bg-violet-950/40"
            >
              Försök igen
            </button>
          </div>
