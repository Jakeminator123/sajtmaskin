...
| F2 render-first advisory | I F2 (`design`) failar ett typecheck-*only*-fel inte versionen: `quality-gate`-routen promotar och svarar `{ passed: true, vmGatePassed: false, designAdvisory: true, advisoryChecks: ["typecheck"] }` (ingen auto-repair i `post-checks.ts`). `next dev` renderar JS trots typfel. Build/lint/verifier/promote-guard förblir hårda; F3 kör alltid `typecheck+build+lint` hårt. Render-säkerheten ägs uppströms av finalize-preflight. |
