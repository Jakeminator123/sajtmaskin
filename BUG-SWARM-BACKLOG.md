...
- Varje aktiv rad har ett stabilt `SM-###`. Nästa lediga ID är `SM-094`.
...
| [ ] | Öppen kodbugg | P1 | `SM-080` Preview-projekt kör i samma filsystem/processidentitet och kan läsa andra projekts filer. | `preview-host/src/runtime/process-lifecycle.js` och `workspace-files.js` på `2566eec511`; syntetisk syskonfil 2026-09-09. Inget nytt korsprojektprov 2026-09-17. | **Blockerar öppen lansering.** Path-jail #1445 (ersatte #1409) landade på preview `a90d9da43` och begränsar intern workspace-skrivning; det är inte sandboxisolering. Beställ inte en tredje jail-rematch. **Riktningen är ratificerad 2026-09-18** — isolerad Fly Machine per kundprojekt, fail-closed vid obevisad isolering; se [`docs/decisions/README.md`](docs/decisions/README.md). Sizing, volymtopologi, idle-timeout och pooling är medvetet otunade och bevisas i pilot. Live Fly-health 2026-09-17 bar `release.sourceSha` `d37011919`, inte #1445. |
...
| [ ] | Delvis åtgärdad | P1 | `SM-072` Chromium-captures svälter `/tmp` på warm Fluid-instans; burst kan döda nästa launch och visa Degraderad trots frisk sajt. | Vercel runtime 2026-08-31 (chat `30840b09`). Kod på master via [#1234](https://github.com/Jakeminator123/sajtmaskin/pull/1234). Ytterligare preview [#1318](https://github.com/Jakeminator123/sajtmaskin/pull/1318). Inget nytt host-/captureprov 2026-09-17. **Ny burst-evidens preview 2026-09-20** (chat `a0b134d9`): sju dumpar på elva timmar — 382, 391, 398, 400, 906, 914 och 979 MB — alla på rutten `/api/engine/chats/[chatId]/product-postcheck`. 906–979 MB är klart över det tidigare intervallet 389–436 MB (2026-09-11→17). Alla sju postcheckar landade ändå `passed`: warningen läggs på i `finally` efter att domen satts, så dumpen degraderar inte sajten. | Återöppna inte accepterade residualer utan ny evidens. Kvar: prod-burst utan `playwright_unavailable` i följd; `/tmp`-race i `src/lib/capture/browser.ts`; ev. burstprov. 2026-09-20-bursten kom från upprepade omkörningar på samma warm-instans (se `SM-093`) — den visar storleksspannet, inte att en enskild capture blivit dyrare. |
...
| [ ] | Behöver aktuell repro | P2 | `SM-093` Flera samtidiga builder-flikar på samma `chatId` kan hålla en version i `pending` genom upprepad `preview_superseded`. Preview-sessionen är en singleton per chat, så varje flik som resynkar flyttar `previewSessionId`/`lifecycleToken`/`filesRevision` under en pågående Product Postcheck. F3 kör om, nästa flik flyttar pekaren igen. Ingen backoff eller retrytak syns i loggen. | Preview 2026-09-20, chat `a0b134d9`, version `db18f6f0` (v6): 126 rader `product_postcheck.summary` med `skippedReason=preview_superseded`, 02:26:13–02:53:54 UTC, därefter `passed` 02:56:40 — ~31 min i `pending`. Samma chats v1–v5 har noll sådana rader. Kodväg: `isExpectedPreviewTargetCurrent` → `getActivePreviewSessionAsync(chatId)` + `matchesExpectedPreviewTarget` i `src/lib/gen/verify/product-postcheck.ts`; `src/lib/hooks/chat/post-checks.ts` noterar att cross-tab bara skyddas av routens per-versions-lease. | Antal flikar var inte kontrollerat — tre var öppna på samma chat. Kvar: repro med exakt två flikar på samma `chatId` och en follow-up, så orsakskedjan blir falsifierbar. Först därefter beslut om åtgärd (tak/backoff på supersede-omkörning, eller att pekarbytet ska avbryta i stället för att lämna `pending`). Bygg ingen ny sessionsmodell på den här raden. |
...
`SM-093` kommer från en read-only prodläsning 2026-09-20 av chat `a0b134d9`
(preview-miljö, PR-preview-deployer mot prod-databasen). Samma läsning gav den
nya burst-evidensen på `SM-072`. Den observerade körningen hade tre samtidiga
builder-flikar på samma chat, vilket inte är ett kontrollerat prov — därför
står `SM-093` som `Behöver aktuell repro` och inte som öppen kodbugg.
