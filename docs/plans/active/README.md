...
| Hydration-fel och reparationskedjan | [`2026-08-05-hydration-reparationskedja/00-master-plan.md`](2026-08-05-hydration-reparationskedja/00-master-plan.md) | Plan skriven 2026-08-05 efter förundersökning i två agentpass (kod friad, detektor-/scaffold-/promptluckor filverifierade). Steg 1–2 levererade via PR #777 (mergad); steg 3 (browser-fel → error-log) i PR från `feat/preview-client-error-log` | Steg 4 (RepairGate-koppling) väntar på steg 3-data; steg 5 (patch-lane settle) väntar på live-repro på Fly |
| Generationslatens | [`2026-08-05-generationslatens/00-master-plan.md`](2026-08-05-generationslatens/00-master-plan.md) | Plan skriven 2026-08-05 på prod-mätning av fyra versioner: strömmen är 79–99 % av väggklockan, bildhämtning kostar noll, och verifierns 69 s var trippel-gatead av BuildSpec-klassningen (inte av autofix-risk — bugbot-fynd korrigerat före merge). Steg 1 levererat i PR från `feat/telemetry-stream-visibility`: `meta` med i telemetri-kinden, `meta.streamMs` **direktmätt** vid stream→finalize-gränsen (brief/orkestrering ingår inte), och `materialize_images`-fasen registreras nu — låst av runner-test | **Steg 2 (BuildSpec-klassningen) kan tas nu** när mätningen är på plats. Steg 3 (verifiern ur F2) är ett ägarbeslut, steg 4 (parallell codegen) en beslutspunkt som kräver ägar-OK |
