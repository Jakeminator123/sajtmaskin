...
import { runDeterministicRepairPrepass } from "@/lib/gen/verify/repair-loop";
...
describe("repairGeneratedFiles — radix unification follows the project manifest", () => {
  ...
  it("honours an explicit verbatimRepo flag over the manifest heuristic", () => {
    const withoutManifest = repairGeneratedFiles([accordionFile()], { verbatimRepo: true });
    const kept = withoutManifest.files.find((f) => f.path === "components/ui/accordion.tsx");
    expect(kept?.content).toContain("@radix-ui/react-accordion");
    expect(kept?.content).not.toContain('from "radix-ui"');

    const forcedOwnEngine = repairGeneratedFiles(
      [manifestFile({ "@radix-ui/react-accordion": "^1.1.2", next: "14.0.3" }), accordionFile()],
      { verbatimRepo: false },
    );
    const unified = forcedOwnEngine.files.find((f) => f.path === "components/ui/accordion.tsx");
    expect(unified?.content).toContain('from "radix-ui"');
    expect(unified?.content).not.toContain("@radix-ui/react-accordion");
  });
});
...
describe("runDeterministicRepairPrepass — radix lane flag", () => {
  function fence(path: string, language: string, content: string): string {
    return "```" + language + ' file="' + path + '"\n' + content + "\n```";
  }
  ...
  it("keeps scoped radix without a package.json when verbatimRepo is true", async () => {
    ...
    expect(prepass.content).toContain("@radix-ui/react-accordion");
    expect(prepass.content).not.toContain('from "radix-ui"');
  });
  ...
  it("unifies for the own-engine lane even with a scoped-only manifest", async () => {
    ...
    expect(prepass.content).toContain('from "radix-ui"');
    expect(prepass.content).not.toContain("'@radix-ui/react-accordion'");
  });
});
