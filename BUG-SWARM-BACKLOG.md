...
MVP-genomlysning 2026-09-09 kompletterar nedan med `SM-078`–`SM-081` mot
`master` `678594c`; äldre rader har inte omverifierats i detta pass. Kodförslag
i preview är inte bevis på att produktionsrisken är stängd.
...
| --- | --- | --- | --- | --- | --- |
| [ ] | Bekräftad prod-bugg | P1 | `SM-078` `wizard_runs` kan läsas och ändras via publika databasroller; en egen aktiv rad kan kringgå wizardavgiften. | Supabase prod 2026-09-09: RLS av, anon/authenticated DML-grants; en datalös REST-läsning returnerade 200. `src/lib/db/migrations/add-wizard-runs.sql` på `678594c`. | Härdningsmigration och rolltester ingår i [MVP-planen](docs/plans/active/2026-09-09-mvp-sakerhet/00-master-plan.md). Tillämpa via kanonisk migrationsväg och verifiera ACL/RLS live innan raden stängs. |
| [ ] | Bekräftad kodbugg | P1 | `SM-079` Google-koppling aktiverar lösenordet från en overifierad förregistrering. | `src/lib/db/services/users.ts` på `678594c`; reproducerad med syntetiskt konto och faktiska service-/loginfunktioner. | Transaktionell koppling och regressionstest ingår i MVP-planen. Promota och verifiera flödet; tidigare redan kopplade konton kräver separat bedömning, eftersom historisk lösenordsverifiering inte kan härledas ur dagens flagga. |
| [ ] | Bekräftad kodbugg | P1 | `SM-080` Preview-projekt kör i samma filsystem/processidentitet och kan läsa andra projekts filer. | `preview-host/src/runtime/process-lifecycle.js` och `workspace-files.js` på `678594c`; faktiska körhjälpare läste en syntetisk syskonfil lokalt 2026-09-09. | **Blockerar öppen lansering.** Separat exekveringsmiljö per projekt med privata filer/hemligheter och nät-/resursgränser; syntetiska acceptansprov finns i `preview-host/README.md`. Release-SHA löser spårbarhet, inte isolering. |
| [ ] | Bekräftad kodbugg | P1 | `SM-081` Parallella genereringar i olika chattar kan passera samma fria/saldobaserade förkontroll innan AI-kostnaden uppstår. | `src/lib/credits/server.ts`, create/follow-up-handlers och efterdebiteringen på `678594c`; chat-låset serialiserar inte användarkontot. | MVP-planen inför kontolås före AI-arbete med ny saldokontroll och avbrottstålig livscykel. Kvar: verklig beloppsreservation och kostnadstak för en enskild körning samt övriga betalda arbetsvägar. |
| [ ] | Bekräftad prod-bugg | P1 | `SM-082` `icon-component-value-fixer` skriver om `{x.icon}` även inne i attribut (`name={product.icon}` → ternary med `<product.icon />`) och inför TS2322/TS2339/TS2604 på korrekt modellkod; fixaren är klassad `safe` så verifieraren skippas. | Prod 2026-09-10 chat `5d809cc1` (ecommerce/megastore-clean): RAG-fyndet `typescript-product-icon-prop` citerar exakt fixarens ternary; Vercel-bygget `dpl_34NEe6vc…` föll på samma fyra diagnostik. `ICON_VALUE_RENDER_RE` i `src/lib/gen/autofix/rules/icon-component-value-fixer.ts` på `33d9ef099` saknar positionsgräns. TS2604 är tredje vanligaste tsc-koden i prod (8/63 på 40 d). | Fix på `fix/reparationskedjan`: bara JSX-barn (`>`/radstart), bara i filer med komponentikon-evidens, `risk: "risky"` i registret, regressionstest med prod-koden. Arkivera vid merge. |
| [ ] | Bekräftad prod-bugg | P1 | `SM-083` En designversion som advisory-promotats med typfel kan publiceras till Vercel; `resolveDeployReleaseGate` släpper allt som inte är `integrations`, men `next build` kör strikt `tsc` och fäller bygget. Ingen automatik fångar `vercel-build-error`. | Samma körning: deploy 17:07 med `pending`, `status=error` 17:08, advisory-promote 17:11 → `verification_state=passed` i UI. `engine-version-lifecycle.ts:199` på `33d9ef099`. 40 d prod: 14 av 19 fallna typechecks advisory-promotade, 1 deploy. | Fix på `fix/reparationskedjan`: `resolveDeployTypecheckAdvisoryGate` (409 `DEPLOY_TYPECHECK_ADVISORY` i deploy-POST, spegelblocker i readiness) läser `resolveLatestGateAdvisoryChecks`; preview och promotering orörda. Arkivera vid merge. |
| [ ] | Bekräftad prod-bugg | P2 | `SM-084` Verify-lanens `tsc` rapporterar `TS1005`-syntaxfel från Nexts genererade `.next/dev/types/routes.d.ts`; grinden räknar det som fallen typecheck och advisory-promotar friska sajter som «klar med varningar». | Prod-signatur `9bf13221eb3e`, 6 chattar 2026-08-13→09-01, 15 av 63 tsc-träffar på 40 d; alla rader pekar på `.next/dev/types/routes.d.ts`. Genererad `tsconfig.json` inkluderar globben (`project-scaffold.ts:113`, Next-paritet). | Fix på `fix/reparationskedjan`: `normalizeTypecheckResult` i `runQualityGateChecks` viker `.next/`-enbart till pass. Kvar (preview-host, Fly-deploy): rensa `.next/dev/types` i verify-workspacet före `tsc` så bruset inte ens uppstår. |
...
| [ ] | Bekräftad prod-bugg | P1 | `SM-074` `preview_ready_timeout`-bannern falsklarmar permanent på frisk sajt. REVIDERAD ROTORSAK 2026-09-01: en follow-up mot en hibernerad VM handoff:ar den GAMLA sessionsidentiteten till den nya versionen (`preview_followup_lane: lane=update, reason=runtime_not_running` → `preview_url_handoff` med gammal `previewSessionId`; `updatePreviewHostSession` hårdkodar `startOutcome: "resumed"`) medan boot:en kommer upp under NY session/lifecycle. Klientens identitetsmatchning kan då aldrig lyckas: deadline fäller bannern, late recovery läser running+mismatch som terminalt och stänger utan att armera self-heal, och inspectorn dör fail-closed på ostämplade/felstämplade bridge-meddelanden. Höjd reload-timeout hjälper inte — ingen reload försöks. | Live-repro 2026-09-01 (chat `c2371f9c`, v3): Vercel-runtimelogg 04:32:34 UTC visar handoff `ps_4d04a764` för v3 medan `/preview-status` från 04:34:30 svarade `running` med `ps_5222ca2b` för samma version; klientens sessionsbärande polls (alla `ps_4d04a764`) upphörde 04:36:11 och bannern stod kvar efter `promoted/passed`. Friska syskon i samma chat: v1 (prewarm) och v2 (update mot levande runtime) — enda skillnadsvariabeln är hibernerad VM. Äldre repro `4cac8fb0` (samma symtom). Utredning: `docs/plans/active/2026-09-01-verifieringsflode-och-inspector/`. | Klientfix mergad [#1232](https://github.com/Jakeminator123/sajtmaskin/pull/1232) (`onPreviewSessionRotated`). Sanningsraden ovanför previewn tas bort i [#1237](https://github.com/Jakeminator123/sajtmaskin/pull/1237). Ägarbeslut 2026-09-01 (chat `5efde3c4`, 14:13): timeout-bannern tas bort helt — `version_mismatch_auto_resync` läkte ytan medan den röda bannern låg kvar över en frisk sajt. Recovery + telemetri (`preview_ready_timeout`, suspect, late recovery, auto-resync) behålls tysta. Kvar: prod-follow-up mot hibernerad VM utan banner; valfri serverhärdning av handoff vid `reason=runtime_not_running`. Delhärdning i `preview` (väntar promote) [#1314](https://github.com/Jakeminator123/sajtmaskin/pull/1314): same-version-rewrite medan hosten bootar (`running=false`, `booting=true` i `files-manifest`) patchas in i pågående boot i stället för `/update` → stop → andra boot (2026-09-08 chat `4a2aa301` v1: ready 20:35:38 → stopp → ready 20:35:51). Hibernerad-VM-fallet (`booting=false`) tar fortfarande `/update`. |
| [ ] | Öppen bug | P2 | `SM-085` Sen product-postcheck/live review av en äldre version läser chattens SENASTE user-prompt som `userRequest` i stället för den prompt som skapade versionen, så granskningen kan döma v1 mot uppföljningens önskemål. | `src/lib/gen/verify/live-review.ts:236` (`pickUserRequest` väljer ur hela meddelandelistan) och `src/app/api/engine/chats/[chatId]/product-postcheck/route.ts:633` (skickar chattens messages utan versionsspärr). Observerad 2026-09-01 (chat `5efde3c4`): en sen resume bedömde v1 mot uppföljningens prissektionskritik. | Pinna `userRequest` till den user-turn som skapade `versionId` (via skapandetid eller versionsmetadata) i stället för chattens sista meddelande. Rotorsaken är en annan än `SM-077`:s uteblivna omkörning. |
| [ ] | Öppen bug | P2 | `SM-086` Desktop-skärmbilden för live review kan bli `null` utan något fynd, och granskningen körs vidare som om båda viewportarna fanns. | `src/lib/gen/verify/product-postcheck.ts:1234` (`capturePostcheckJpeg` sväljer throw → `null`) och `src/lib/gen/verify/live-review.ts:128` (`hasCurrentScreenshots` är OR mellan desktop och mobile). Observerad 2026-09-01 (chat `5efde3c4`): v1 fick `has_desktop: false` medan v2/v3 fick båda. | Varna när `desktopUrl` saknas, och kör inte live review som «båda viewportar» på enbart mobil — alternativt retry:a desktop-capture en gång. |
| [ ] | Bekräftad kodbugg | P3 | `SM-089` Init-turens plan-läge i `create-chat-stream-post.ts` skrev inga `plan_mode_turn_entry`/`plan_mode_turn_exit`-rader; bara uppföljningsturen (`plan-mode-turn.ts`) lämnade spår. Första plan-turen i en chat var därför osynlig. | Prod 2026-09-15 chat `f550445e-717e-42f0-8e36-e2f17669084d`: tre `plan_mode_turn_exit`, init-turen saknades och rekonstruerades ur DB-rader. `create-chat-stream-post.ts` saknade anrop till `plan-mode-trace.ts`. | Stängs i denna PR: samma entry/exit-kontrakt på init, inklusive kastande planner. Arkivera vid merge till `master`. |
| [ ] | Bekräftad prod-bugg | P1 | `SM-088` Plan-läge dör tyst efter besvarade frågor: `Godkänn plan och bygg` (`BuildPlanCard` → `handleApproveBuildPlan` → `buildApprovedPlanExecutionPrompt`) renderas bara när `showStructuredChat` är på. Default är av, och togglen *Felsökningsvy* syns bara med `?debug=1`. Frågorna kommer via `pendingReply`; när blockers=0 försvinner den ytan och ingen version startar. Live-chatten visar samtidigt 5–7k tecken plannerström. | Prod 2026-09-14/15 chat `f550445e`: tre `plan_mode_turn_exit` med `outcome=plan_persisted`; sista `hasPlanArtifact=true`, `hasBlockers=false`, persist «Plan skapad (brochure)…»; 0 rader i `engine_versions`/`engine_generation_logs`. `useBuilderState.ts` default `false`; `MessageList.tsx` gömde `type: "plan"` bakom `showStructuredParts`; `BuilderHeader.tsx` kräver `?debug=1`. Inte `planner-text` (`083a4eaa` 2026-08-23). Inte `785c8d7a` (tyst persistmiss). | Fix på `fix/sm-088-plan-cta`: `BuildPlanCard` renderas oberoende av felsökningsvyn; live-content byts mot `buildPlanSummaryMessage` vid plan-`done`. Regression: plan redo + `showStructuredChat=false` → knappen syns. Arkivera vid merge till `master`. |
| [ ] | Bekräftad prod-bugg | P2 | `SM-077` Sen preview-boot ger ingen omverifiering: `product-postcheck.ts` klassar boot-splashen som icke-blockerande timing (rätt), men när VM:n kommer upp efteråt körs ingen ny kontroll, så en version kan få `Readiness: PASS`/`passed` och sedan krascha vid första riktiga render utan att reparationslagret (autofix/live review) någonsin ser det. | Live-observation 2026-09-08 (chat `fc197819`, `/logg-internet`-körning 22:16): sajt 1 stod ~20 min på `warm_project`; v2 fick PASS och kraschade därefter på `Icon is not defined` (oimporterad ikonkomponent från follow-up) utan reparation. Prod-defektsignatur `a9bf697e5ca8` («F2 Product Postcheck superseded — versionen lämnas pending», 5 förekomster/2 chattar sedan 2026-09-04). Hör ihop med `SM-072`/`SM-074`. | Kör om postchecken (eller minst runtime-/console-delen) när `preview-status` går `starting → ready` efter ett `preview_boot_page`-utfall, så att `runtime_crash` når autofix. Fånga `Icon is not defined`-klassen i import-validatorn för follow-ups (kandidat: samma `import-validator`-lane som init). |

MVP-prioritering: `SM-078`–`SM-081` före öppen lansering. Övrig prioritering:
`SM-072`, `SM-073` och `SM-074` först; därefter `SM-088` (plan-läge dör tyst),
`SM-077`, `SM-085` och `SM-086`, `SM-033`, `SM-013`, `SM-003` och `SM-001`; sist
`SM-030`.
...
| P2 | Kostnadsfri: vilka bolagsfält får JakobScrape-dashen pusha in i `extra_data`? Källan bär personnummer och ledamöters hemadresser, och `extra_data` når både browsern och LLM-prompten. | Innan `profile`-fältet byggs. |
| P3 | Kostnadsfri: bara preview-prewarm vid lösenordsverifiering, eller även spekulativ init-generering före mini-wizarden? Entitlementet finns redan vid verifiering och konsumeras först vid lyckad version. | Fritt. |
