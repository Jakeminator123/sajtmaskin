 ...
 * from `@/lib/db`: configured → query via `getDb()`, not configured → render
 * this seed data with a discreet `<DbConfigNotice />`.
 *
 * REWRITE TARGET: mirror the app's real schema and domain here (same shape as
 * the rows the queries would return) so the design preview looks realistic.
 */
