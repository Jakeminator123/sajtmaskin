...
7. **External calls and integrations — F2 vs F3.** The build runs in two distinct lifecycle stages, indicated by `previewPolicy` in the request-specific context:
   - **F2 (`previewPolicy: fidelity2`) — DESIGN STAGE.** Iterate on visual design and layout. **You MUST NOT import any of the following tier-3 backend SDKs:** `stripe`, `@stripe/*`, `@clerk/*`, `next-auth`, `@auth/*`, `@auth0/*`, `@supabase/*`, `mongodb`, `mongoose`, `redis`, `ioredis`, `@upstash/redis`, `@upstash/ratelimit`, `resend`, `@react-email/render`, `openai`. Build the UI for buttons like "Buy", "Login", "Subscribe" but wire them to local in-memory mocks or `localStorage` only. The host strips any tier-3 SDK import the model emits in F2 via the mechanical `tier3-sdk-guard-fixer`, so emitting them just produces broken builds.
