...
### Route-tabs add/remove page (PR #224, 2026-06-23) — Bugbot-fynd

Manuell Bugbot-review av #224 efter merge (Codex var rate-limitad vid PR-tillfället). Flag-gated (`NEXT_PUBLIC_SAJTMASKIN_QUICK_EDIT`). Båda fynden rör samma designspänning: reachability-filtret döljer onåbara route-tabs medan add/remove-flödet inte garanterar nåbarhet. Mildrade (add-toast varnar + chat är workaround), men reella UX-återvändsgränder. Kräver ett **produktbeslut**: dölja orphan-routes (ren fliklista) vs visa dem (badge + `−`, men brusigare).

> **Löst i `fix/route-tab-orphans` (produktval: visa)** — `derivePreviewRoutes` flaggar nu `reachable` per route i stället för att filtrera bort orphans; chrome listar olänkade routes med "olänkad"-badge + `−`-knapp (och de är navigerbara om statiska). BB-1: en add-page utan auto-nav blir synlig + öppningsbar. BB-2: orphan/onåbara sidor är nu borttagbara via UI. Reachability-*logiken* (subtree-exkludering) oförändrad.

- [ ] **BB-1 (high): add-page som ej kan auto-länka nav blir osynlig.** När `handleAddPage` lyckas men `buildAddNavLinkOps` ger `navUpdated:false` ([PreviewPanel.tsx](src/components/builder/preview-panel/PreviewPanel.tsx)) persisteras sidan, men `derivePreviewRoutes` ([preview-route-helpers.ts](src/components/builder/preview-panel/preview-route-helpers.ts):326-334) släpper den (inget utanför dess subtree länkar dit efter reachability-fixen) → success-toast men ingen flik/öppningsväg. Mildring: toasten säger "be i chatten att länka sidan". Fix-alternativ: (a) garantera en nav-länk vid add (fallback-placering), eller (b) visa nyss skapade men onåbara sidor som egen "olänkad"-flik.
- [ ] **BB-2 (medium): onåbara/orphan-sidor kan ej tas bort via UI.** `−`-kontrollen renderas bara för `previewRoutes` (nåbara) ([PreviewPanelChrome.tsx](src/components/builder/preview-panel/PreviewPanelChrome.tsx):425-471), men orphan-sidfiler döljs av reachability-filtret — de blockerar ändå dubblett-add via `routeHasPageFile` och saknar `−`-knapp → fastnar tills användaren editerar via chat. Fix: visa orphan-routes (badge) med `−`, eller en separat "rensa olänkade sidor"-åtgärd.
...
>
> **Uppdatering 2026-06-23 (MB-3 fix):** MB-3 är nu **fixad**. Codegen-anropet (`pipeline.model`) + SSE/version-telemetri routar generator-fasen via `resolvePhaseModel(tier, "generator")` i alla tre call-sites, så `anthropic`-tier genererar på `claude-opus-4.8`. Persisterad `chat.model` lämnas medvetet som tier-build-modellen (`resolveEngineModelId`) så repair/server-verify round-trippar tier via `ownModelIdToCanonicalModelId`. Övriga tiers oförändrade i default-config.
>
> **Uppdatering 2026-06-23 (MB-3 follow-up, Bugbot-fynd):** Bugbot fångade att `engineModelId` (som `deriveBuildSpec` skalar token-budget mot via `getModelContextWindowTokens`, `orchestrate.ts:1275`) fortfarande pekade på tier-build-modellen → Opus-genereringen fick Sonnets kontextfönster-budget på `anthropic`-tier. **Fix:** codegen-`orchestrationInput.engineModelId` i `create-chat-stream-post.ts` + `chat-message-stream-post.ts` använder nu `generatorModel`. Se ny rad **MB-4** för plan-mode (pre-existing, separat).
...
- [x] **MB-3 (medium, pre-existing) — FIXAD 2026-06-23: Anthropic generator-fas-modell var inert i normal generering.** Tidigare skickade de vanliga builder-streamsen `pipeline.model = engineModel = resolveEngineModelId(tier)` = `buildProfiles.defaults.anthropic` (`claude-sonnet-4.6`), så Opus-4.8-generatorn aktiverades aldrig vid val av Anthropic-tier. **Fix:** `pipeline.model` + version/SSE-telemetri använder nu `resolvePhaseModel(tier, "generator").modelId` i `create-chat-stream-post.ts`, `chat-message-stream-post.ts` och `generate-site-from-prompt.ts`. `chat.model` (createChat) lämnas som tier-build-modellen så `repair/route.ts` + `server-verify.ts` round-trippar tier via `ownModelIdToCanonicalModelId`. Endast `anthropic`-tier ändrar beteende; övriga tiers är identiska i default-config (generator == build-default). **Follow-up (Bugbot):** även `orchestrationInput.engineModelId` (token-budget-skalning) använder nu `generatorModel` i codegen-vägarna.
- [ ] **MB-4 (low, pre-existing): plan-mode token-budget skalas mot tier-build-modellen, inte planner-fasen.** I plan-läge sätts `engineModelId: resolveEngineModelId(resolvedModelTier)` på plan-`orchestrationInput` ([create-chat-stream-post.ts](src/lib/api/engine/chats/create-chat-stream-post.ts) plan-grenen + [chat-message-stream-post.ts](src/lib/api/engine/chats/chat-message-stream-post.ts) plan-grenen), medan plannern faktiskt kör `resolvePhaseModel(tier, "planner").modelId` (för `anthropic` = `claude-opus-4.8`). `deriveBuildSpec` skalar därför planner-kontexten mot Sonnets fönster på Anthropic-tier. Gapet fanns före MB-3 (planner-routing till Opus är pre-existing). **Fix:** sätt `engineModelId: planModel` i plan-`orchestrationInput` (i `create-chat` är `plannerSettings.modelId` redan beräknat före plan-orchestration; i `chat-message` krävs att `resolvePlanModePlannerSettings` flyttas upp före plan-`orchestrationInput`). Låg prio: plan-läget genererar plan-artefakt, inte kod.
