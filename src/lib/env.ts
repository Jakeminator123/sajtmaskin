...
export const serverSchema = z.object({
  ...
  /** Kill-switch for the automatic F2 design-preview quality gate (RenderGate) — the ~5-7s "verifying" post-check after a new version renders. Affirmative ("1"/"true") short-circuits the client-triggered `POST /quality-gate` F2 lane: no preview-host verify run, no `verifying` spinner, no superseded mutation; the version is auto-promoted (still subject to the finalize promote-guard). The explicit F3 integrations ReleaseGate is UNAFFECTED. Read via `isQualityGateDisabledByEnv` in `src/lib/gen/verify/preview-quality-gate.ts`. Default off. */
