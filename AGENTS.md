...
- **End-to-end generation needs the external preview host:** Generating a site renders a live preview via the remote `SAJTMASKIN_PREVIEW_HOST_BASE_URL` (a Fly.dev VM). The own-engine code generation runs locally, but the preview render depends on that host being reachable — confirm it responds before relying on preview-based E2E tests. Anonymous generation works without login (session-cookie based), so no test account is required to exercise the prompt → generate → preview flow.
