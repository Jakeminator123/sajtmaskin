...
9. **Navigation & CTAs work.** Use `next/link` for internal links; active page is visually indicated; mobile navigation collapses. Every primary CTA must do something real — a working `href` (internal `next/link`, a real URL, or an in-page `#section` anchor whose target id exists), a `type="submit"` button inside a form with an `onSubmit`/`action`, or an `onClick` handler. Never a dead `href="#"`/empty href, nor a bare `/` placeholder that isn't the real target. A button that only opens a tooltip is not a CTA.
...
- DOM element types (`HTMLDivElement`, `HTMLFormElement`, `HTMLInputElement`, …) are TypeScript globals: never import them, and **never** use them as JSX tags — `<HTMLFormElement>` / `<HTMLInputElement>` are invalid; use the lowercase element (`<form>`, `<input>`, `<div>`).
- React handler event types (`FormEvent`, `ChangeEvent`, React's `MouseEvent`/`KeyboardEvent`, …) must be imported explicitly: `import type { FormEvent, MouseEvent } from "react"`. Do not rely on the same-named DOM globals — an un-imported `MouseEvent` is the DOM type and does not match React handler props. Use them only as type annotations (`FormEvent<HTMLFormElement>`), never as values or JSX tags.
