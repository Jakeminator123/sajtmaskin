...
| Knip-driven unused-export purge (~35 exports totalt över flera rundor) | Downgraded till module-local eller borttagna (buildPreviewUrl, isBuildSpecEnabled, resolveLatestVersions, getAllDossiers, getDossierStatus, NON_RUNTIME_STATUSES, LEGACY_ALIAS, LEGACY_MODEL_IDS, EMPTY_VERIFIER_FINDINGS, promoteForcedBlockingFindings, isServerVerifyInFlight, resolvePlanModePlannerModelId, _resetTier3DenyCacheForTests, m.fl.). |
| **Duplikat-städning promptAssist.ts ↔ guidance-resolvers.ts** | 5 byte-identiska funktioner (`buildDomainStructureHints`, `buildDomainContractHints`, `inferMotionProfile`, `resolveMotionGuidance`, `resolveQualityBarGuidance`) + 7 keyword-banks (`MOTION_STATIC_*`, `MOTION_LIVELY_*`, `MOTION_GUIDANCE`, `QUALITY_BAR_GUIDANCE`) bodde i båda filerna. `guidance-resolvers.ts` är nu canonical; `promptAssist.ts` importerar. Shrinks promptAssist 1144 → 815 rader (−329). |
| `prompt-assist-models.ts` extraherad | Assist-model allowlist (`ASSIST_MODELS`, `ANTHROPIC_ASSIST_MODELS`, normalize/isOpenAI/isAnthropic/isOff/isAllowed/resolveProvider) bodde mitt i `promptAssist.ts`. Callers som bara vill veta "vilken provider?" kan nu importera från den 58-radiga modulen istället för att dra in ~800 rader prompt-bygg-kod. `promptAssist.ts` re-exporterar för bakåtkompat. |
