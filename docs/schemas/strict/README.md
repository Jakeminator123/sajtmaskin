...
| `agent-workflow.schema.json`             | `config/agent-workflow.json`                                                                                                     | `scripts/workflow/*` + GitHub workflow contracts                                                                 | CI/local tooling: `npm run workflow:contract`. No product runtime read.                                                                                                                     |
| `openclaw-builder-job-spec.schema.json`  | Server-owned `BuilderJobSpec` envelope                                                                                           | `src/lib/openclaw-builder/builder-job-spec.ts`                                                                    | Schema mirror checked with AJV parity fixtures; product runtime uses the adjacent Zod owner.                                                                                                |
| `scaffold-variant.schema.json`           | `config/scaffold-variants/<scaffoldId>/*.json`                                                                                   | `src/lib/gen/scaffold-variants/types.ts`                                                                         | Backoffice validate-on-save + blocking `backoffice:test`; runtime parser in `registry.ts`                                                                                                   |
