...
| K-016 | `27pct-w`, `31pct-t` | `chat-area.tsx`: wireframe → `landing-wireframe-shapes.tsx` (**del 1**); radar + Lighthouse → `landing-comparison-radar.tsx` + `landing-lighthouse-gauges.tsx` (**del 2**); kvar: tech/integration/feature-kort, modal, fallback, `ChatArea` | refaktor | låg–medel | [ ] |
