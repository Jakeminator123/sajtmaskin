...
export const siteBriefSchema = z.object({
  ...
  requestedCapabilities: z
    ...
    .max(12)
    .describe(
      "Real requested capability ids in kebab-case, e.g. auth, payments, database, media-storage, ai-chat, booking, visual-3d, physics-3d, carousel, command-palette, site-search, map-display, gallery-lightbox, dashboard-charts, contact-form, newsletter-subscribe. There is ONE auth capability (`auth`) — the provider (Clerk default / Supabase on explicit ask) resolves downstream. Plain content sections (FAQ, pricing, testimonials, logo row, stats, CTA, multi-step form UI) are page content, not capability ids. Do not list generic page sections like hero, about, contact, footer, or SEO.",
    ),
  pages: z
    .array(
      z.object({
