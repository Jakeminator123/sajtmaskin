...
- **Root-absoluta Next-interna requests (`/__nextjs_font/*`, `/_next/*`)**: Next dev-overlay/devtools begar egna assets (t.ex. `geist-latin.woff2`) pa hostroten UTAN chatId-prefix och struntar i basePath. Proxyn tolkade tidigare `__nextjs_font` som chatId → generisk JSON-404. Nu finns en Referer-fallback (`nextInternalRefererFallback` i `runtime.js`, HTTP + WS-upgrade): chatId hamtas fran Referer-headerns forsta path-segment och requesten proxas till `/{chatId}{originalPath}`. Utan Referer (strikt referrer-policy) faller den fortfarande till 404 — kosmetiskt, overlayn tar systemfont.
