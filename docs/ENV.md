...
| Lokal körningslogg (genlogg) | `MAX_GEN_LOGS=10`, `OPENAI_ADMIN_KEY`, `DID_API_KEY` | Läses **bara** av `scripts/observability/last-generated-usersite.py` (lokalt verktyg) — inte av appen, så de hör inte i `serverSchema` och ska inte sättas i Vercel. `MAX_GEN_LOGS` = antal körningsmappar som sparas under `data/gen-logs/` (default 10, äldsta raderas). `OPENAI_ADMIN_KEY` är en **admin**-nyckel (`sk-admin-…`, Settings → Organization → Admin keys) som ger läsning av hela organisationens usage/costs — vanlig `OPENAI_API_KEY` duger inte, och nyckeln ska hållas lokal. `DID_API_KEY` är D-ID:s server-API-nyckel (`API_USER:API_PASSWORD`) för credits-saldot; de publika `NEXT_PUBLIC_AVATAR_*` är client-nycklar och räcker inte. Alla tre är valfria — saknas de hoppas den källan över. Detaljer: [`scripts/observability/README.md`](../scripts/observability/README.md). |
