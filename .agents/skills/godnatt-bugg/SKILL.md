---
name: godnatt-bugg
description: Drive one or more Sajtmaskin BUG-SWARM backlog items from fresh revalidation through app-isolated-worktree implementation, independent review, pull request, merge gate, cleanup, and cooldown. Use when the user invokes $godnatt-bugg, writes /godnatt-bugg or "godnatt bugg", asks for one or N autonomous backlog bug passes, or when a scheduled Godnatt-bugg tick resumes an armed batch. Do not use for general audits or bugs outside BUG-SWARM-BACKLOG.md.
---

# Godnatt bugg

Orkestrera exakt ett säkert pass per task. Behandla backloggen som hypoteser,
inte facit. Läs alltid repo-rotens AGENTS.md och de regler som den routar till
innan någon åtgärd.

## Tolka kommandot

- $godnatt-bugg eller /godnatt-bugg: skapa ett pilotpass med count 1.
- $godnatt-bugg N utan ordet full: avvisa och förklara att merge-mandat måste
  vara bokstavligt.
...
full-batch och count som användaren redan har armerat.
...
kommandon, promotion och automationslivscykel.
...
6. Kör queue och acquire. Vid busy/cooldown/paused/completed: gör ingen mutation
   och följ automationslivscykeln.
...
Already-resolved och reclassify räknas först efter mergad PR och cleanup-handoff.
...
Flytta till ci-review och sedan ready-to-merge. State kräver en clean eller
findings-fixed review för exakt aktuell head-SHA.
