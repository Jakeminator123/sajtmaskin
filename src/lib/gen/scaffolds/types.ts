...
export const SCAFFOLD_CLIENT_LIST: ReadonlyArray<{
...
}> = [
  {
    id: "base-nextjs",
    label: "Base Next.js",
    description: "Minimal Next.js starter with Tailwind, App Router, and dark theme.",
...
/**
 * Scaffolds that users may pin explicitly from the Builder's "Välj själv"
 * menu. The Scaffold: Av baseline remains in `SCAFFOLD_CLIENT_LIST` so the
 * client projection keeps exact registry parity and can still label persisted
 * or diagnostic state, but it is an implementation detail rather than a
 * separate manual choice.
 */
export const MANUALLY_SELECTABLE_SCAFFOLD_CLIENT_LIST = SCAFFOLD_CLIENT_LIST.filter(
