...
| Parkerad dossier-identitet | F2 sparar både `mutedCapabilities` (behovet) och `mutedDossierIds` (exakt provider-syskon, t.ex. `supabase-auth`). `Bygg integrationer` godkänner dessa identiteter durabelt innan F3, så ett generiskt knappmeddelande aldrig får byta Supabase mot capabilityns standardval Clerk. |
