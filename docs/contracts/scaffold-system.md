...
`npm run backoffice` startar Streamlit-appen (`sajtmaskin_backoffice.py`). Byggstenar-ytor (omdöpta 2026-07-24, gamla `?nav=`-slugs fungerar fortfarande): **Byggstenar: översikt** (mental modell, renderad ur glossary + kontraktsdocs), **Scaffolds: titta & justera** (översikt + termguide + detaljer + metadata-redigering), **Scaffolds & varianter: skapa, redigera, klona, ta bort** (skapa/redigera/klona/radera + varianter + baseline-återställning; efter skapande/redigering måste matchning publiceras till Blob via index-knapparna), **Guide: ny scaffold eller variant (AI)** (AI-guidat skapande; integritetsklar först när embeddings publicerats och `scaffolds:validate` är grön), **Byggblock (dossiers)** och **Mallar (v0): inspiration & uppladdning**. Telemetri-poängen bor i **Scaffold-poäng** (gruppen Telemetri & loggar). Alla sparningar säkerhetskopieras (sidan **Återställning**).
