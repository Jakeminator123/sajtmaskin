...
|---|---|---|---|---|---|
| Prompt formatting | fallback-wrap (`MÅL` + `TILLGÄNGLIGHET`) när brief hoppas över; ingen LLM. Inte i `useCreateChat`-init | `src/lib/builder/prompt-assist/formatters.ts` | rå användarprompt | formatterad prompt + snabb addendum | torftig prompt förblir för lös |
| Assist-modell (hint) | `promptAssistModel` väljer modell till Deep Brief. Inte Polish, inte `/api/ai/chat`-rewrite (knappen borttagen 2026-04-21). Live brief-steg: [`llm-role-matrix.md`](llm-role-matrix.md) | `src/lib/hooks/useInitBrief.ts`, `meta.promptAssistModel` | request-meta | modell-hint | leftover `promptAssistMode` polish/rewrite är död kod |
| Simple website path | konservativ snabb init-lane för korta website/template-prompts utan brief, attachments, custom system, multi-route, integrations/contracts eller capabilities | `src/lib/api/engine/chats/simple-website-path.ts`, `create-chat-stream-post.ts`, `orchestrate.ts` | rå prompt + keyword pre-match scaffold + capability flags + named dossier-capabilities | `simpleWebsitePath=true/false` + devLog `orchestration.simple_website_path`; när true: ingen Server Auto-Brief, inga externa/UI Recipes, ingen dossier selection | för bred predicate skulle tappa brief/dossiers; därför blockerar både heavy flags och den delade named-capability-detektorn |
