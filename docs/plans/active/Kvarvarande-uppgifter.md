...
| 11 | **Event-bus UI-flip** | Delvis hårdat 2026-05-01: `VersionHistory` skiljer nu runtime/preview-badge från verifieringsbadge (`Design redo`/`Verifierar`/`Verifierad`). Kvar: koppla `selectVersionStatus(events)` in i `BuilderShellContent.tsx` + preview-panel SSE-handling så UI inte längre härleder allt från DB-flaggor. | Medel | — |
