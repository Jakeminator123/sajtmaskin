...
import {
  WixAlternativContent,
  readyRelatedSeoLandingSlugs,
} from "./wix-alternativ-content";
...
describe("WixAlternativContent", () => {
  it("renders one primary heading, product CTA and only ready related links", () => {
    const entry = getSeoLandingEntry("wix-alternativ");
    render(<WixAlternativContent />);
    ...
    expect(body).toContain("Välj Wix om");
    expect(body).toContain("Överväg Sajtmaskin om");
    expect(body).toContain("Ingen one-click-flytt");
    ...
    expect(screen.queryByRole("link", { name: /wordpress|lovable/i })).toBeNull();

    const readyRelated = readyRelatedSeoLandingSlugs(entry.relatedSlugs);
    expect(readyRelated).toEqual(
      expect.arrayContaining(["hemsideprogram", "ai-hemsidebyggare", "skapa-hemsida"]),
    );
