...
| Observability metrics (`/api/metrics`) | Sedan 2026-04-20: `src/lib/observability/metrics.ts` exponerar Prometheus-metrics via `prom-client@^15`. Singleton-Registry cachas på `globalThis.__sajtmaskinMetricsRegistry` (Next dev-mode hot-reload-säker). Helpers: `recordPhaseDuration(phase, durationMs)` / `incFixerCall(name, outcome)` / `incVerifierBlocking(findingId)` / `incPartialFileRepair(outcome)` / `incEarlyStop(reason, phase)` / `recordPromptToDone(ms, outcome, kind)`. 12 kanoniska faser i `OBSERVED_PHASES` (utan `pre_vm_typecheck` — uppgår i `validate_syntax`). Wirad i `validate-and-fix.ts` (phase duration + early-stop), `verifier-pass.ts` (phase duration + per-finding blocking-counter), `finalize-version.ts` (partial-file-repair outcome), samt `create-chat-stream-post.ts` + `chat-message-stream-post.ts` (P50 via `withPromptToDoneMetricResponse`/`prompt-to-done-stream.ts` som transparent `TransformStream` detekterar SSE `event: done`-bytes för att mäta end-to-end). Alla call-sites fail-safe (telemetri kan aldrig bryta codegen). Custom-metric-prefix `sajtmaskin_*`; Node default-process-metrics aktiverade. Endpoint: `GET /api/metrics` med `Authorization: Bearer <SAJTMASKIN_METRICS_TOKEN>` eller `?token=<X>`-query (tom env = 503 disabled). Avlåser audit Tier A #12/#16 + §3.1-data. | kanonisk |
