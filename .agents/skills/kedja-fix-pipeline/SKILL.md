...
**Expensive orchestrator? Delegate.** The default is one `<terra>` runner for steps 1–6; the parent receives only the final table and runs step 7. Every intermediate report is re-paid on later turns, so keep it out of the parent context.
...
4. **Models from the canonical rule** in [`subagent-models.mdc`](../../../.cursor/rules/subagent-models.mdc): `<luna>` for read-only localisation; `<terra>` for repro, fixes, runner and Bugbot. Sol only via the rule's escalation gate.
...
### Step 2 — repro agent (1 agent, writes, `<terra>`)
...
### Step 5 — fix agents (N parallel, write, `<terra>`)
...
`subagent_type: "bugbot"`, `readonly: true`, `description: "Bugbot"`, `model: "<terra>"`, prompt form per `AGENTS.md` review guidance:
