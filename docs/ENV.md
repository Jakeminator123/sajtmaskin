...
| **Genererad användarsajt i Vercel Sandbox** | Tillfällig Next-app som **builder** startar med användarens kod; **inte** samma process som Sajtmaskin | Autentisering mot Vercel för **Sandbox API**: `VERCEL_OIDC_TOKEN` (rekommenderat efter `vercel link` + `vercel env pull`) **eller** `VERCEL_TOKEN` + `VERCEL_TEAM_ID` / `VERCEL_ORG_ID` + `VERCEL_PROJECT_ID`. **Preview-läge i sandbox:** `SAJTMASKIN_SANDBOX_PREVIEW_MODE` (`dev_only` default, `dev_then_build`, `build_only`) — styr om `npm run build` körs i sandlådan efter dev. **Readiness:** `SAJTMASKIN_SANDBOX_READINESS_MAX_MS` (valfri, millisekunder, default 90000) — max väntan på att dev-servern svarar HTTP efter `npm run dev`. **Innehåll i den genererade appen:** sammanslagen `.env.local` från placeholders + `project_data.meta.projectEnvVars` (se `src/lib/gen/build-generated-site-env.ts`); det är **inte** samma som att lägga nycklar bara i Sajtmaskins `.env.local`. |
