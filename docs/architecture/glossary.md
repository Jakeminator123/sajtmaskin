...
| Brief (`meta.brief`) | Strukturerat JSON-objekt (~24 fält: projectTitle, pages[], visualDirection, toneAndVoice, imagery, seo + designguidance-fält). Bär designintention. Genereras av LLM via `/api/ai/brief` (init) eller `tryGenerateServerAutoBrief` (server-fallback). `briefQuality`: `"full"`, `"server-auto"`, eller `"none"`. Follow-ups återkör inte Deep Brief-LLM:en, men `buildFollowUpBriefFromSnapshot` (A1/A2, 2026-04-21 + styleKeywords/tone + `domainProfile`-slug 2026-04-22) hydrerar en minimal snapshot-brief från `orchestration_snapshot.briefSummary` så `requestedCapabilities`, `domainProfile`, `visualDirection.styleKeywords` och `toneAndVoice` lever över init→follow-up utan ny LLM-rundgång. | kanonisk |
...
| Capability (dossier) | Abstrakt sak sajten kan göra — strängvärde i brief (`brief.requestedCapabilities = ["payments", "ai-chat", …]`). Varje dossier deklarerar exakt en capability i `manifest.json`. Capability matchas 1:1 mot dossier (default-tie-break via `defaultForCapability`). Ej att förväxla med "Capability Map" nedan som gäller fysik/3D/auth-detektion i scaffolds. | kanonisk |
...
| Locale Alternate Routes | `/contact`↔`/kontakt`, `/about`↔`/om`, `/services`↔`/tjanster`, `/blog`↔`/blogg` (lagts till 2026-04-21). `deduplicateLocaleAlternateRoutes` i `route-plan.ts` behåller den som matchar projektets resolved locale (default `sv`). **Sedan 2026-04-21 wirad in i `buildRoutePlan()` så LLM:n aldrig ser båda varianterna samtidigt** — tidigare var helpern deklarerad men oanvänd. |
| Href Route Cross-Check | Deterministisk preflight-check (`crossCheckHrefsAgainstRoutes` i `src/lib/gen/verify/href-route-cross-check.ts`, sedan 2026-04-21) som extraherar `<Link href>`, `href=""`, `router.push()`, `redirect()` ur genererade `.tsx`/`.jsx` och flaggar interna hrefs som inte matchar någon faktisk route. Template literals (`` `/blogg/${slug}` ``) godkänns mot motsvarande dynamisk route. Suggestions via Levenshtein ≤ 2. Severity initialt `warning`; gate-flip till `error` planerad i `docs/plans/active/repair-loop-hardening.md`. | kanonisk |
| Canonical Route Paths | Strikt sektion i system-prompten (sedan 2026-04-21) som listar exakta `routePlan.routes`-paths LLM:n får använda i navigation. Komplement till route-plan-dedupen och spegling av `Href Route Cross-Check`. | kanonisk |
...
| Version-Mismatch Overlay | UX-mekanism för perioden mellan att en ny version sparas och att preview-VM:en hunnit reload:a. `VersionMismatchOverlayPayload`-typ exporteras från `preview-host-client.ts` (sedan P24, 2026-04-20). Konsumenten landade i P25b-rest 2026-04-21 (komponent + `PreviewPanelFrame.versionMismatchPayload`-prop) — se separat rad `VersionMismatchOverlay` nedan för komponent-detaljer. **Dispatch-vägen** (poll/SSE som detekterar mismatch och fyller payload i `PreviewPanel.tsx`-callern) är inte wirad ännu; tills den finns renderar overlayen aldrig i normalflödet. | kanonisk |
...
| `integrationsBuild` (lane) | F3 quality gate. `["typecheck", "build", "lint"]`. Används vid promotion / "Bygg integrationer"-flödet. | kanonisk |
...
| Fixer Registry (`FIXER_REGISTRY`) | Single source of truth för alla ~40 fixers (mekaniska + LLM + verifier) i `src/lib/gen/autofix/fixer-registry.ts`. Varje entry har `id`, `category`, `sourcePath`, `targetFailureMode`, `triggers`, `status`, `ownerPhase`, valfri `telemetryCounter` och `notes`. Parity-test (`fixer-registry.test.ts`) garanterar invariants. Backoffice-sida `Fixer Registry` läser snapshot:en `data/observability/fixer-registry.snapshot.json`. Snapshot regeneras vid `npm run dev|build|start` eller manuellt via `npm run fixers:dump`. | kanonisk |
| Repair-Pass-Index Pruning (`pruneStaleVersionErrorLogs`) | Best-effort delete av rader i `engine_version_error_logs` med `meta.repairPassIndex < currentRepairPassIndex` när follow-up/repair-finalize på samma `versionId` är clean. Adresserar SAJ-25-symptomet "röd Fel-badge på fungerande preview" där error-log ackumulerades över passes. Gated på `FEATURES.consistentRepairPassIndex`. Telemetri: `version_error_log_pruned { droppedCount, reason }`. | kanonisk |
| Verifier Re-Run After Fix | Efter `runLlmFixer` lyckas på verifier-blocking findings re-körs `runVerifierPass` ÉN gång på den patchade contenten för att bekräfta att fixet löste problemet. Capad på 1 re-run + 30 s timeout. Gated på `FEATURES.verifierRerunAfterFix`. Telemetri: `verifier_rerun_after_fix { before, after, durationMs }`. Ersätter den optimistiska `verifierBlockingFindings = []`-clear som tidigare ljög "fixed" till UI:t även när inget hade fixats. | kanonisk |
| Recurring Failures Block | `### Recurring failures on this site`-block i system-prompten (Phase 2D). Renderar topp-5 `RecurringFailurePattern` (≥ 2 occurrences) från `readRecurringPatternsForChat(chatId)` så HUVUDgeneratorn (inte bara fixer) ser vad den just gjorde fel. Capat till 600 chars, faller tyst om budget slut. Bara på follow-ups. Gated på `FEATURES.recurringPatternsInMainPrompt`. | kanonisk |
| Error-Log RAG | Vector RAG över historiska fault/fix-events. Producer (`appendErrorLogEvent` i `src/lib/logging/error-log-rag.ts`) skriver NDJSON i `logs/llm-segmentts-and-index/error-log.ndjson`. Indexer (`scripts/observability/index-error-log-rag.mjs`) bygger deterministisk TF-IDF-snapshot i `data/observability/error-log-tfidf-snapshot.json` (delta-aware, 5k-row-tail). Retriever (`retrieveSimilarFailures` + `renderErrorLogRagBlockLines`) injicerar `### Lessons from similar past builds` i system-prompten. Auto-ingest hook i `scripts/dev/next-runner.mjs` triggar reindex vid `npm run dev|build|start` (5s timeout, fire-and-forget). Gated på `FEATURES.useErrorLogRag`. Backoffice-sida `Error-log RAG` exponerar producer-status, indexer-meta och topp-fault-kategorier. | kanonisk |
...
| Tier3BuildSpec | Strukturerat F3-byggkontrakt (`src/lib/integrations/tier3-build-spec.ts`): per integration `requiredRealEnvKeys` (filtrerat till `enforcement: "build"`), `placeholderOkEnvKeys`, `featureRuntimeEnvKeys`, `warnOnlyEnvKeys`, `buildInstructions[]`, `setupGuide`. Renderas som `## Tier-3 Integration Build Plan` i F3-system-prompten. P31 utökade typen + lade till `allowPlaceholdersForBuildKeys`-option på `validateTier3Readiness`. | kanonisk |
...
| Dossier System (v2) | Sedan 2026-04-20: deterministic capability-driven dossier-urval. `data/dossiers/{hard,soft}/<id>/` ersätter den tidigare 96-dossier-poolen + embedding-pipen. `selectDossiersForRequest` matchar `brief.requestedCapabilities` 1:1 mot dossiers (en per capability, eller noll). Inga embeddings, ingen domain-veto, inga kategori-cap. Hard-class kollar env-vars och flaggar `configured`-status; unconfigured triggar placeholder-UI istället för krasch. Källa: `docs/architecture/dossier-system.md`. Gammal pipe + 96-dossier-pool ligger i `archive/dossiers-legacy-2026-04-20/`. | kanonisk |
...
| F2 quality-gate `build`-check | **Reverterad 2026-04-23.** `qualityGateTiers.designPreview` är åter `["typecheck"]`. `build` och `lint` flyttades till pre-VM warm-cache-passen i Sajtmaskin-backend (`src/lib/gen/preview/warm-typecheck.ts` + `src/lib/gen/preview/warm-eslint.ts`) som körs innan filerna skickas till preview-host. Samma diagnostik, men utan Fly-CPU-kostnad och med LLM-fixer-loop inline. Mellantid 2026-04-20 → 2026-04-23 körde F2 `["typecheck", "build", "lint"]` på VMn som mellanlösning (Audit Tier S #7 / `01-buggar.md` §1.5). F3 (`integrationsBuild`) har alltid `["typecheck", "build", "lint"]`. Revert-sökväg: sätt `designPreview` i `config/ai_models/manifest.json` till det längre setet om du behöver VM-build-skyddsnätet igen. | kanonisk |
...
| SAJTMASKIN_SHIM_PREVIEW_DISABLED | Env-flagga som stänger av legacy "compatibility shim"-previewen helt. **Default sedan Block D: ON (shim avstängd).** Operatörer kan återaktivera shim genom att sätta variabeln till `0`/`false`/`off`/`no` — alla andra värden (inkl. unset eller `1`/`true`/`yes`/`on`) håller shim avstängd. När shim är avstängd returnerar `buildPreviewUrl()` `null`, `/api/preview-render`-routen svarar `410 Gone` med diagnostic `render_route_shim_disabled`, och `isShimOrMissingPreviewUrl()` behandlar alla icke-tier-2 URLs som "behöver upgrade" så bootstrappen re-runs om en stale legacy-URL slipper igenom. UI:t hanterar avsaknad av preview-URL via `PreviewPanelEmptyState`. Tier-2 VM-previewen (Fly) påverkas inte. Implementerad i `src/lib/gen/preview/legacy/compatibility-shim.ts` (`isShimPreviewDisabled()` helper). | kanonisk |
| `preview-url-classifier.ts` | Modul `src/lib/gen/preview/preview-url-classifier.ts` som äger **tier-2-relaterad URL-klassificering**: `isTier2LivePreviewUrl`, `hasTier2LivePreviewUrl`, `normalizePreviewUrl`, `resolveAlternatePreviewUrls`, `AlternatePreviewUrls`-typen + host-suffix-läsning från `NEXT_PUBLIC_SAJTMASKIN_TIER2_PREVIEW_HOST_SUFFIXES` (med `fly.dev` som default). Bröts ut ur `legacy/compatibility-shim.ts` i Block D1 (2026-04-21) så att F1-shim-helpers (`isShimPreviewDisabled`, `isCompatibilityShimPreviewUrl`, `isShimOrMissingPreviewUrl`, `OWN_ENGINE_PREVIEW_PATH`) kan retireras utan att stör tier-2-detektionen. Importera tier-2-helpers härifrån; importera shim-helpers från `compatibility-shim.ts`. | kanonisk |
| `project-env-events.ts` | Modul `src/lib/builder/project-env-events.ts` som är **enda** dispatch-källan för `CustomEvent("project-env-vars-open")` och `CustomEvent("integrations-panel-open")` i builder-UI:t. Skapad i Block C1 (2026-04-21) för att ersätta 4 inline-implementationer i `BuildPlanCard` / `LaunchReadinessCard` / `VersionDiagnosticsDialog` / `BuilderShellContent` (`BuilderMessageTooling.openProjectEnvVarsPanel` är kvar som re-export för bakåtkompat). SSR-säkra (early-return på `typeof window === "undefined"`). Listener: `ProjectEnvVarsPanel`. F2-mute-not: panelen mountas bara i F3, så dispatcher i F2 är tysta no-ops by design — anropare ska gate sina knappar på `lifecycleStage === "integrations"` för att inte visa inerta CTA:er. | kanonisk |
| `VersionMismatchOverlay` | React-komponent `src/components/builder/preview-panel/VersionMismatchOverlay.tsx` som är konsumenten för `VersionMismatchOverlayPayload` (typen från `preview-host-client.ts`). Renderas via `PreviewPanelFrame.versionMismatchPayload`-prop (default `null`); visas över iframen när appen och preview-VM är osynkade på vilken version som körs. **Stacking:** `z-30` över loading-overlayen (`z-10`), men suppressas av `PreviewPanelFrame` när `iframeError` är true så error-state alltid vinner. **Pointer-events:** blockerar iframe-interaktion (`absolute inset-0`) avsiktligt — interaktion med en stale mid-boot-preview är värre än att vänta. **Dispatch-vägen** (poll/SSE som detekterar mismatch och fyller payload) är *inte* implementerad — komponenten är redo att ta emot payload när någon wirar dispatch-kanalen. Stängde P25b-rest 2026-04-21. | kanonisk |
| `eval-baseline-update` (workflow) | GitHub Actions-workflow `.github/workflows/eval-baseline-update.yml` (workflow_dispatch + cron Mon 04:11 UTC) som auto-refresh:ar `src/lib/gen/eval/eval-baseline.json`. Kör `cli.ts --gate --save-baseline` i en enda invokation (en eval-körning, en OpenAI-billing-event), och om baseline-filen ändrats öppnas en **draft-PR** via `peter-evans/create-pull-request@v7`. Push direkt till master är medvetet undvikt — baseline-shifts kan dölja regressioner om de accepteras blint, så operatör måste granska PR:en manuellt. Hard-fails om `OPENAI_API_KEY`-secret saknas. | kanonisk |
...
| `content-site` (scaffold-id) | Slagen ihop med `landing-page` 2026-04-23 (OMTAG fas 2·B / M1). Tidigare en marketing-scaffold för multi-section brand storytelling (`siteKind: marketing`, 1 variant `warm-editorial`); övertygande överlapp med `landing-page` (samma `siteKind`, `complexity: medium`, samma intents, delad keyword-lista). Sammanslagningen: registry- och typborttagning, `LANDING_KEYWORDS` absorberar gamla `CONTENT_KEYWORDS`-banket, `warm-editorial` och `minimalist-mag` flyttade till `landing-page/`. Före/efter-audit: `OMTAG/fas2-B-audit-before-after.md`. |
