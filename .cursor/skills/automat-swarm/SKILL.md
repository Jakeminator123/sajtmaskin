...
5. **Keep volume cheap.** Models come from the canonical table in [`.cursor/README.md § Modellval för subagenter`](../../README.md#modellval-för-subagenter-kanonisk-tabell): `composer-2.5-fast` for scan and distill rounds, `cursor-grok-4.5-high` for falsification rounds only. Never invent a slug — an invalid one does not fail loudly and may run on the (expensive) parent model.
...
|---|---|---|---|---|
| 1, 3, 5 … | **Scan** | 8, one lane each | `composer-2.5-fast` | find new candidates in rotating lanes |
| 2, 4, 6 … | **Falsification** | one per unverified finding, max 8 | `cursor-grok-4.5-high` | try to prove the finding is **not** a bug |
...
2. **Launch.** Parallel `Task` calls (`subagent_type: explore`, `readonly: true`, `model: cursor-grok-4.5-high`) with the falsification prompt below.
