 ...
 *
 * Generated from lucide-react 0.469.0 runtime exports (the generated project
 * baseline in `project-scaffold.ts`), excluding duplicate `*Icon` aliases.
 * This must be broad enough that valid icons such as `Cpu` are preserved
 * instead of being routed through fuzzy fallback logic.
 *
 * IMPORTANT: This list must match the lucide-react version used by generated
 * projects, not merely this repo's dev dependency. Brand/semantic aliases still live in
 * LUCIDE_BRAND_ICON_REPLACEMENTS below and intentionally win before exact
 * runtime export matches.
 *
 * To update: regenerate from the generated-project baseline lucide package exports.
 */
