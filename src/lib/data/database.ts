...
import fs from "fs";
import { PATHS, logConfig } from "@/lib/config";
...
// ============================================================================
// Single admin account with unlimited credits.
...
export const TEST_USER_PASSWORD = SUPERADMIN_PASSWORD;
export const TEST_USER_DIAMONDS = SUPERADMIN_DIAMONDS;
...
// Ensure admin account exists - works in ALL environments
// Creates a single admin user with unlimited diamonds and superadmin flag
function ensureTestUserExists(database: Database.Database) {
  ensureAdminExists(database);
}

// Ensure admin exists - creates superadmin account
// Admin has unlimited diamonds and is_superadmin flag set
...
export function canUserGenerate(userId: string): {
  allowed: boolean;
  remaining: number;
  limit: number;
} {
  ...
  if (user?.is_superadmin || user?.email === TEST_USER_EMAIL) {
    return {
      allowed: true,
      remaining: DAILY_RATE_LIMITS.generations,
...
export function canUserRefine(userId: string): {
  allowed: boolean;
  remaining: number;
  limit: number;
} {
  ...
  if (user?.is_superadmin || user?.email === TEST_USER_EMAIL) {
    return {
      allowed: true,
      remaining: DAILY_RATE_LIMITS.refines,
