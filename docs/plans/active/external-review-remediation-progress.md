...
**Final sweep / handoff (2026-03-28):** `npm run typecheck` och `npx vitest run` (88 filer, **387** tester) ska vara gröna efter varje kodbatch. Valfritt: `npm run test:deploy-smoke:e2e` (skippas utan env). Otrackade kataloger som `data/`, `logs/` lämnas utanför commit (se `.gitignore` + [`repo-hygiene.md`](../architecture/archive/pre-2026-03-consolidation/repo-hygiene.md) § *Git versus Cursor*). **Handoff efter remediation-exit:** [REMEDIATION-EXIT.md](../avklarat/external-review-execution/REMEDIATION-EXIT.md) + [kritik-consolidated-open-items.md](./kritik-consolidated-open-items.md) för produktbacklog.
...
| **`1.txt`** — del B | **Integrationer + runtime-flöde** (registry, detektion, manifest, env, lansering, deploy-API) | **~83%** | HTTP-e2e runt deploy (auth); **K-007 [x]** 2026-03-26 (`deploy-precheck.md`); `e2e/README` + Vitest-kontrakt; fler providers vid nytta |
| **`2.txt`** | **Own-engine** (stream-routes tunna, session, finalize, golden tests, v0-gräns, **fas→modell** `B3-02`) | **~81%** | **K-009 [x]/[N/A]** 2026-03-26 — [`own-engine-sse-scope.md`](../../architecture/own-engine-sse-scope.md); ny SSE → ny plan |
...
2. **Staging:** `git add <filer>` lägger ändringar i **index** (”staging area”) — bara staged filer följer med i nästa `git commit`. `git status` visar vad som är staged vs endast ändrat lokalt. Stagea endast reporelevanta filer (inte lokala `data/`, `logs/`). **Undantag:** avsiktlig **kritik-/arkiv-hygien** under `.j_to_agent/archive/` och flytt av färdigställda `kritik/*.md` ska med när batchen är dokumenterad (se § *Done* + `KRITIK-OVERVIEW.md`).
...
|--------|------|-----------|---------------------------|
| **Whole vision** (syntes av tre dokument + tvärgrepp) | **100%** *remediation-exit* | **produktbacklog** (K-018/K-019 + valfri deploy-e2e; K-007/K-009/K-008/K-014 stängda i kritik) | Se [REMEDIATION-EXIT.md](../avklarat/external-review-execution/REMEDIATION-EXIT.md) |
| **Landing slice** (steg 1–4 i `1.txt`, delvis) | **~96%** | **~4%** | **`1.txt` del A**; W1-track kryssat i MASTER |
| **Integrationer + deploy** (`1.txt` steg 5–7) | **~83%** | **~17%** | **`1.txt` del B**; manifest + 409 + Vitest; `deploy-precheck` (**K-007 [x]**); **preview/`iframe`** (K-018) |
...
- **Ägarprioritet preview + mall-distinktion (2026-03-25):** **K-018** ny (användarsidor: React som `npm run dev`, fidelity i **`iframe`**); **K-008 [x]** (landning fryst); **K-009** scope i `own-engine-sse-scope.md` (**[x]/[N/A]** 2026-03-26); FAQ + `e2e/README` — **Vercel-templates = scaffolds** för OwnEngine, **V0-templates** separat; `PLAN-KRITIK-OPEN`, `COMPLETION-ROADMAP`, `KORFIL`, `REMAINING-WORK`, `kritik-consolidated`, progress.
- **Ägarbeslut B–I (2026-03-26, doc-batch):** **K-014 [x]** (juridik/cookies/om oss OK oförändrat); `queue/FRAGOR-SVAR-FAQ.md` (B1/C1/D1/I1, `e2e/` vs v0); Plan 17 — F1 v0 separerat, G1b ENV låg prio, H1c research + H2c `docs/old` → `avklarat/2026-03-docs-old-archive/`; `e2e/README.md` § *TL;DR*; synk `KORFIL`, `COMPLETION-ROADMAP`, `PLAN-KRITIK-OPEN`, `PLAN-REPO-SEPARATION-OPEN`, `kritik-consolidated`, `REMAINING-WORK`.
...
- **Remediation exit (2026-03-28, orchestrator-run):** [REMEDIATION-EXIT.md](../avklarat/external-review-execution/REMEDIATION-EXIT.md); valfri Playwright-smoke `e2e/deploy/deploy-api-precheck.smoke.spec.ts`; `playwright.deploy-smoke.config.ts`; `npm run test:deploy-smoke:e2e`; `e2e/README.md` + progress/MASTER/kritik-batch. *Run arkiverad lokalt:* `2026-03-28-external-review-remediation-exit`.
...
- **W1 / K-008 delmoment (2026-03-27, orchestrator-run):** `landing-how-it-works-lazy.tsx` — WebGL först vid in-view; reduced-motion → statisk fallback; `chat-area.tsx` terminalmarkör respekterar reduce. **`deploy-precheck.md`** § *Produktbeslut K-007* (auto-fix oförändrad, 2026-03-26). **`sitemap.ts`** — JSDoc-checklista vid nya marknadssidor. *Lokal run:* `2026-03-27-external-review-final-pct` → arkiverad.
- **Gen / route-plan (2026-03-27):** `route-plan.ts` — promptmönster **om oss** → **`/om`** (`Om oss`); engelska **about** / **company** / **story** → **`/about`**; `route-plan.test.ts`. **Cursor-regel:** [`.cursor/rules/agent-intent-board.mdc`](../../../.cursor/rules/agent-intent-board.mdc) § *Push*.
...
- **B3-05 / W4 (2026-03-27):** `scripts/extract-static-core.mjs` borttaget — monolitisk `STATIC_CORE` i `system-prompt.ts` finns inte; statisk kärna via `static-core-loader` + `config/prompt-static/`. Uppdaterat: `scripts/README.md`, `docs/architecture/archive/pre-2026-03-consolidation/prompt-tree.md`, `buglista-del-3.md`, `track-w4-scripts.md`.
...
- **W2 / deploy-kontrakt (2026-03-26, K-007 delmoment → K-007 [x] 2026-03-26):** Vitest för **`precheckOnly`** utan `skipAutoFix` (auto-fix körs) **och** `precheckOnly` + `skipAutoFix`; `docs/architecture/archive/pre-2026-03-consolidation/deploy-precheck.md` § **Kontraktstester** + § *Produktbeslut K-007*.
...
- **W3 (namngivning):** `createGenerationPipeline` flyttad till **`src/lib/gen/generation-pipeline.ts`**; `src/lib/gen/fallback.ts` re-exporterar för äldre importvägar. Stream-routes, MCP `generate-site`, Vitest-mocks och `run-eval` needles uppdaterade; `docs/architecture/archive/pre-2026-03-consolidation/v0-soft-deprecation.md` justerad.
...
- W2 deploy-hårdning (2026-03-25): **`docs/architecture/archive/pre-2026-03-consolidation/deploy-precheck.md`** beskriver auto-fixar + **opt-out** (`skipAutoFix` / `SAJTMASKIN_DEPLOY_DISABLE_AUTO_FIX=1`); **`POST /api/v0/deployments`** ger **409** (`DEPLOY_MISSING_ENV`) om obligatoriska env saknas efter preflight; valfri body **`precheckOnly`** för torrkörning utan credits.
