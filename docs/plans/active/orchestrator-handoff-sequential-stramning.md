...
- **Öppen fråga (diskuteras separat, inte lösa nu):** `vercel_templates_levels/` — “spökmapp” (gitignore/cursorignore, Playwright `references:discover*` vs spårad spec under `e2e/`). Nästa agent efter denna run fortsätter det samtalet; i denna run bara **notera** om du rör sökvägar så det dokumenteras i loggen.
...
- Nästa steg efter denna run: **fortsätt diskussion om `vercel_templates_levels/`** med användaren; ev. en dedikerad workload för “kanonisk Playwright-sökväg + docs”.
