...
**`sourceTemplateIds`:** alla variants pekar på **riktiga v0-mall-Blob-id:n** i `src/lib/templates/template-blob-manifest.json` (legacy-slugs från den borttagna external-template-pipelinen remappades 2026-07-22). Listan är en ordnad kandidatpool; init och `clear-redesign` väljer **högst en** mall via `template-inspiration.ts`. Användbara preview-kompatibla kandidater är primär kohort och rangordnas deterministiskt mot prompt + Deep Brief; användbara addendumutdrag premieras och källordningen bryter lika resultat. En användbar kandidat med `previewFits:false` är fallback enbart när kohorten är tom — inspirationsvägen läser aldrig arkivet, så mallen duger som bild-/kodinspiration, men den får inte tränga ut ett val som inte var trasigt. `selectionReason` bär `cohort=preview-fit|preview-fallback`. Tillåtna helprojektskategorier är `landing-pages`, `website-templates`, `apps-and-games`, `dashboards`, `login-and-sign-up`, `e-commerce` och `blog-and-portfolio`. `ai` väljs normalt inte, med ett explicit granskat undantag för `h4nibkqysVJ` (AEGIS-Ω), vars id, kategori och arkiv-SHA måste matcha den låsta runtime-allowlisten. `animations`, `components`, `layouts` och `design-systems` väljs inte, och ett id i en sådan kategori är död konfig som integritetsgrinden och Backoffice-sparningen avvisar (`never-selectable-template`). Den valda stillbilden skickas som style-only visionreferens. Frontendutdragen hämtas från det SHA-bundna `config/variant-template-addenda.json`. Ett `hit` ger högst tre, totalt 9 000 tecken långa utdrag (huvudsida, direkt komponent, global CSS/layout). `disabled` är kuratorns dom över hela mallen: den väljs aldrig i någon kohort — en användbar kandidat vinner alltid (även en fallback), och är alla kandidater disabled får varianten ingen mallinspiration alls (i dag bara `base-nextjs/starter-neutral`). `missing`, `stale` och `invalid` är dataproblem: de väljs, ger inga kodutdrag och hämtar inte Blob-ZIP:en i användarflödet, men stillbilden skickas. `projekt-bas-app` (Scaffold: Av) resolver aldrig inspiration och har tomma `sourceTemplateIds`. Scaffold, brief, routes och kontrakt äger fortfarande implementationen; mallens brand, assets, paketversioner, routes och backend antas aldrig. Fristående komponent-/blockinspiration väljs separat av UI Recipes. Arkivläsning hör till offline-kommandot `templates:addenda` och till verbatim-import (`POST /api/template`); extraktorn ligger i `template-structural-extractor.ts` och addenda-cachen är fingerprintad över just den modulen, så en urvalsändring invaliderar inte utdragen.
...
| Agentregler      | `.cursor/rules/scaffold-rules.mdc`, `.cursor/rules/repo-router.mdc`                                                                                     | Lista vid sammanslagning                                          |
